---
source_file: "src/pages/Accounty/Ev/EvInnovationLevyPage.tsx"
type: "code"
community: "Accounty Generaterequestemail Updatemessagebody"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Accounty_Generaterequestemail_Updatemessagebody
---

# EvInnovationLevyPage.tsx

## Connections
- [[EvInnovationLevyPage()]] - `contains` [EXTRACTED]
- [[cn()]] - `imports` [EXTRACTED]
- [[evCalculations.ts]] - `imports_from` [EXTRACTED]
- [[formatHuf()]] - `imports` [EXTRACTED]
- [[index.ts]] - `imports_from` [EXTRACTED]
- [[useAccountyClient()]] - `imports` [EXTRACTED]
- [[utils.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Accounty_Generaterequestemail_Updatemessagebody