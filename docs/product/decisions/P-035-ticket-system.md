...
- Kezelőkonzol (Console View): Support munkatársak számára optimalizált 2-hasábos osztott nézet (`TicketsPage.tsx`).
  - **Függőben lévő jegyek szűrése & „Összes jegy” kapcsoló:** A bal oldali jegyqueue (`consoleTickets`) a táblázathoz hasonlóan alapértelmezetten kizárólag a **gazdátlan (nyitott, `assigned_to === null`)** és a bejelentkezett support munkatárs **saját kezelt (`assigned_to === user.id`)** jegyeit jeleníti meg. A lista fejlécében elhelyezett **„Összes jegy”** jelölőnégyzettel (`Checkbox`) a szűrés egyetlen kattintással feloldható a teljes csapat összes folyamatban lévő jegyére.
  - **Informatív üres állapot:** Ha a support adminnak épp nincs saját vagy nyitott jegye, a sáv egyértelműen jelzi: *„Nincs saját vagy nyitott függőben lévő hibajegy”*.
...
- Olvasottság-kezelés életciklusa (`TicketDetailView` & `useMarkTicketRead`):
