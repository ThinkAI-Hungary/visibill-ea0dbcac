...
6. **Vízszintes Görgetősávok Méretvédelme és Szabályos DOM Beágyazás:**
   - **Méretösszeomlás Védelem (`min-h-*` + `shrink-0`):** Amikor egy `overflow-x-auto` görgethető sáv egy `flex flex-col` szülő közvetlen gyermeke, kötelező a `shrink-0` és egy explicit minimális magasság (pl. `min-h-[3.5rem]`). Ennek hiányában a böngésző a scroll konténer méretét 0-ra (csak a paddingre) számolhatja, és az `items-center` miatti eltolás levágja a gombok felső/alsó felét (lásd: [A-150](../architecture/decisions/A-150-gl-performance-resilience-and-layout-hardening.md)).
   - **Gomb Beágyazási Tilalom (Valid DOM Nesting):** Tilos `<button>`-be más interaktív elemet (pl. `<Switch>`, másik gombot) ágyazni. Ha egy teljes sor kattintható (pl. accordion), a külső elemet `role="button"` + `tabIndex={0}` tulajdonságú `<div>`-ként kell megvalósítani, Enter/Space billentyűkezeléssel és `e.stopPropagation()` védelemmel a belső vezérlőkön.
