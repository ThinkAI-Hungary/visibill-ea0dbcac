...
* **Heurisztikus matching:** Első körben a bizonylatszám, a pontos összeg (cross-currency tűréshatárokkal), a dátum-toleranciák és a partnernév alapján párosít.
  ...
  * **Szigorú szóhatár-alapú számlaszám illesztés és előtag-ütközés védelem (A-128):** A számlaszámok illesztése szigorú alfanumerikus token-határ vizsgálattal (`(?<![a-zA-Z0-9])` és `(?![a-zA-Z0-9])`) történik. Ez garantálja, hogy egy hosszabb bizonylatszám (pl. `RABA-2026-309`) ne adhasson hamis pozitív egyezést a benne prefixként szereplő rövidebb számlákra (pl. `RABA-2026-30` vagy `RABA-2026-3`).
  * **Részhalmaz-kiszűrés és a leghosszabb egyezés elve (Subsumption Filter):** Ha egy közleményben több számlaszám-kifejezés is illeszkedne, de az egyik normalizált alakja szigorú részhalmaza vagy előtagja egy hosszabb talált számlaszámnak, a rendszer a rövidebb találatot automatikusan eldobja.
...
* **Multi-match, Részfizetés és Jóváhagyási Kapu:** A kapcsolatokat a `transaction_invoice_matches` összekötő tábla tárolja (egy tranzakcióhoz több számla, vagy egy számlához több részletfizetési tranzakció is tartozhat).
  * **Multi-Match Jóváhagyási Kapu (Approval Gate — A-128):** Ha a közlemény alapján több független számla kerül összerendelésre egyetlen banki tétellel, a rendszer ellenőrzi a számlák bruttó végösszegének egyezését a banki tétel összegével. Ha az összegek egyeznek, a tétel automatikusan hitelesített (`is_verified = True`, score: 0.95). Ha az összeg eltér, a megbízhatóság fixen `0.85`-re korlátozódik (< 0.90), a tétel nem válik automatikusan ellenőrzötté, és a felületen `jóváhagyásra vár` jelöléssel jelenik meg.
  * **Részfizetés (Partially Paid) Szabály (A-082):** A számla csak akkor minősül teljesítettnek (`paid = true` és `'matched'`), ha a kapcsolt tranzakciók összege eléri a számla bruttó összegét (`paid_amount >= gross_amount - 0.5 HUF`). Részösszeg esetén a számla státusza `'partially_paid'` (kék badge, hátralévő összeg kijelzése).
  * **Életciklus és Visszavonási Integritás (DB Triggerek):** A `transaction_invoice_matches` összekötő sor törlésekor (`trg_reset_paid_on_multi_match_delete`), illetve a `transactions.matched_invoice_id` lekapcsolásakor vagy átváltásakor (`reset_paid_on_transaction_unmatch`) adatbázis-triggerek garantálják, hogy a lekapcsolt számla automatikusan visszanyerje fizetetlen állapotát (`paid = false, transaction_id = NULL`), ha nem kapcsolódik hozzá más aktív tranzakció.
...
- **ADR:** [A-128: Szigorított Számlaszám Határ-illesztés és Multi-Match Jóváhagyási Kapu](../../architecture/decisions/A-128-strict-invoice-number-boundary-matching-and-subsumption-guard.md)
