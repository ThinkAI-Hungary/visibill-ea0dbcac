...
BEGIN
  ...
  WHERE ni.company_id = p_company_id AND ni.invoice_direction = p_direction
    -- Main date filter: delivery date (teljesítési dátum)
    AND ni.invoice_delivery_date >= p_date_from AND ni.invoice_delivery_date <= p_date_to
    -- Optional issue date filter (kibocsátási dátum)
...
-- 2. get_filtered_submitted_invoices (version WITHOUT payment_method)
...
BEGIN
  ...
  WHERE i.company_id = p_company_id AND i.invoice_direction = p_direction
    -- Main date filter: delivery date (teljesítési dátum)
    AND i.teljesites_datuma >= p_date_from AND i.teljesites_datuma <= p_date_to
...
-- 3. get_filtered_submitted_invoices (version WITH payment_method)
