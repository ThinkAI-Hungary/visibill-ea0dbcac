-- Migration: 20260924100000_add_invoice_partner_and_vat_gl_numbers.sql
-- Description:
--   Add partner_gl_number and vat_gl_number columns to invoices and nav_invoices tables.
--   Enforces Sztv. Hungarian double-entry rules:
--   - For outbound (customer) invoices: buyer GL is restricted to 311, 312, 315, 316, 317.
--   - For VAT GL: 467 (payable) and 466 (deductible).
--   - Updates acc_generate_drafts_from_ledger to prioritize invoice-level partner_gl_number and vat_gl_number if present.

ALTER TABLE public.invoices
  ADD COLUMN IF NOT EXISTS partner_gl_number VARCHAR(16) DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS vat_gl_number VARCHAR(16) DEFAULT NULL;

ALTER TABLE public.nav_invoices
  ADD COLUMN IF NOT EXISTS partner_gl_number VARCHAR(16) DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS vat_gl_number VARCHAR(16) DEFAULT NULL;

CREATE INDEX IF NOT EXISTS idx_invoices_partner_gl ON public.invoices(company_id, partner_gl_number);
CREATE INDEX IF NOT EXISTS idx_nav_invoices_partner_gl ON public.nav_invoices(company_id, partner_gl_number);
...
DECLARE
  ...
  -- Invoice-level partner and VAT GL account overrides
  v_inv_partner_gl VARCHAR(16);
  v_inv_vat_gl VARCHAR(16);
  v_eff_cust_gl_id UUID;
  v_eff_supp_gl_id UUID;
BEGIN
  ...
  LOOP
    ...
    ELSIF v_row.source_table IN ('invoice_items', 'nav_invoice_items') THEN
      ...
      IF v_final_direction = 'OUTBOUND' THEN
        v_eff_cust_gl_id := v_gl_cust_id;
        IF v_inv_partner_gl IS NOT NULL AND v_inv_partner_gl IN ('311', '312', '315', '316', '317') THEN
          SELECT id INTO v_eff_cust_gl_id
            FROM public.gl_accounts
           WHERE (preset_id = p_preset_id OR company_id = p_company_id)
             AND gl_number LIKE v_inv_partner_gl || '%'
           ORDER BY gl_number ASC
           LIMIT 1;
          IF v_eff_cust_gl_id IS NULL THEN
            v_eff_cust_gl_id := v_gl_cust_id;
          END IF;
        END IF;

        IF v_eff_cust_gl_id IS NULL OR v_row.gl_account_id IS NULL THEN
          CONTINUE;
        END IF;
        ...
        IF NOT v_is_credit THEN
          INSERT INTO public.acc_journal_lines (
            ...
            v_header_id, 1, v_eff_cust_gl_id, 'T', v_huf_gross, v_foreign_gross, 'NONE', COALESCE(v_row.description, '')
          );
        ...
        ELSE
          ...
          IF v_huf_vat > 0 AND v_selected_vat_gl_id IS NOT NULL THEN
            ...
            INSERT INTO public.acc_journal_lines (
              ...
              v_header_id, 3, v_eff_cust_gl_id, 'K', v_huf_gross, v_foreign_gross, 'NONE', COALESCE(v_row.description, '')
            );
          ELSE
            INSERT INTO public.acc_journal_lines (
              header_id, sequence_number, gl_account_id, dc_type, amount, foreign_amount, vat_role, description
            ) VALUES (
              v_header_id, 2, v_eff_cust_gl_id, 'K', v_huf_gross, v_foreign_gross, 'NONE', COALESCE(v_row.description, '')
            );
          END IF;
        END IF;
      ...
      ELSE
        v_eff_supp_gl_id := v_gl_supp_id;
        IF v_inv_partner_gl IS NOT NULL AND v_inv_partner_gl LIKE '454%' THEN
          SELECT id INTO v_eff_supp_gl_id
            FROM public.gl_accounts
           WHERE (preset_id = p_preset_id OR company_id = p_company_id)
             AND gl_number LIKE v_inv_partner_gl || '%'
           ORDER BY gl_number ASC
           LIMIT 1;
          IF v_eff_supp_gl_id IS NULL THEN
            v_eff_supp_gl_id := v_gl_supp_id;
          END IF;
        END IF;

        IF v_eff_supp_gl_id IS NULL OR v_row.gl_account_id IS NULL THEN
          CONTINUE;
        END IF;
        ...
        IF NOT v_is_credit THEN
          ...
          IF v_huf_vat_deductible > 0 AND v_selected_vat_gl_id IS NOT NULL THEN
            ...
            INSERT INTO public.acc_journal_lines (
              ...
              v_header_id, 3, v_eff_supp_gl_id, 'K', v_huf_gross, v_foreign_gross, 'NONE', COALESCE(v_row.description, '')
            );
          ELSE
            ...
            INSERT INTO public.acc_journal_lines (
              ...
              v_header_id, 2, v_eff_supp_gl_id, 'K', v_huf_gross, v_foreign_gross, 'NONE', COALESCE(v_row.description, '')
            );
          END IF;
        ELSE
          ...
          INSERT INTO public.acc_journal_lines (
            ...
            v_header_id, 1, v_eff_supp_gl_id, 'T', v_huf_gross, v_foreign_gross, 'NONE', COALESCE(v_row.description, '')
          );
