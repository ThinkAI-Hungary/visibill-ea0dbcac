-- Migration: 20260924105500_include_all_suppliers_in_vat_return_m_lines.sql
-- Description: Ensures all supplier partners from whom goods/services were purchased appear in 65M sheets (vat_return_m_lines).
-- Fixes:
-- 1. Removes restrictive 'COALESCE(ni.invoice_vat_amount, 0) > 0' which dropped 0% VAT / AAM / TAM / FAD partners (e.g. Tóth Mária, Bakos Györgyi, GRIGIO 97).
-- 2. Normalizes supplier tax numbers by stripping non-digits before taking the 8-digit base, fixing suppliers with 'HU' prefix (e.g. HU14120228).
-- 3. Groups invoice items per invoice first so invoice_details does not duplicate rows.
-- 4. Unions manual inbound invoices from the 'invoices' table so no supplier is missed.
...
BEGIN
  ...
  -- (regardless of 0% VAT, AAM, TAM, FAD, negative credit notes, or tax number formatting)
  WITH all_inbounds AS (
    ...
    SELECT
      ...
      COALESCE(
        SUM(ROUND(nii.net_amount * (COALESCE(nii.deductible_percentage, 100.0) / 100.0) * COALESCE(er.rate, 1.0), 2)),
        ROUND(COALESCE(ni.invoice_net_amount, 0) * COALESCE(er.rate, 1.0), 2)
      ) AS net_amount,
      COALESCE(
        SUM(ROUND(nii.vat_amount * (COALESCE(nii.deductible_percentage, 100.0) / 100.0) * COALESCE(er.rate, 1.0), 2)),
        ROUND(COALESCE(ni.invoice_vat_amount, 0) * COALESCE(er.rate, 1.0), 2)
      ) AS vat_amount,
      COALESCE(
        SUM(ROUND((nii.net_amount + nii.vat_amount) * COALESCE(er.rate, 1.0), 2)),
