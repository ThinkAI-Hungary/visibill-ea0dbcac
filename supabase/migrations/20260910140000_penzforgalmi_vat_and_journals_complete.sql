-- Migration: 20260910140000_penzforgalmi_vat_and_journals_complete.sql
-- Description: Complete cash-basis VAT support across VAT return calculation (2665) and double-entry accounting journals (47993 / 3689)

-- 1. Update HOPSI Kft. vat_regime to 'penzforgalmi'
UPDATE public.companies
SET vat_regime = 'penzforgalmi',
    updated_at = now()
WHERE id = 'd04c25cc-c3d3-46d0-adb2-f7105bc3402b';

-- 2. Update calculate_vat_return RPC to accurately handle cash-basis settlement dates
...
BEGIN
  ...
  FOR inv_rec IN
    ...
    LEFT JOIN transactions t ON t.id = ni.transaction_id
  ...
  LOOP
    ...
    v_is_paid := (COALESCE(inv_rec.payment_method, '') = 'CASH' OR inv_rec.transaction_id IS NOT NULL OR inv_rec.manual_payment_date IS NOT NULL);
    ...
    AND (
      ...
      CASE 
        WHEN (v_is_penzforgalmi OR COALESCE(ni.is_cash_accounting, false) = true) AND COALESCE(ni.payment_method, '') <> 'CASH' THEN
          (ni.transaction_id IS NOT NULL OR ni.manual_payment_date IS NOT NULL)
...
-- 3. Update acc_generate_drafts_from_ledger RPC to support 47993 and 3689 Cash-Basis VAT
...
BEGIN
      ...
      IF v_matched_inv.id IS NOT NULL AND v_matched_inv.vat_amount > 0 AND COALESCE(v_matched_inv.payment_method, '') <> 'CASH' AND v_journal_ve_id IS NOT NULL THEN
        ...
        IF v_matched_inv.invoice_direction = 'OUTBOUND' AND v_is_penzforgalmi THEN
          INSERT INTO public.acc_journal_headers (
            ...
            v_date, v_date, 'AFA-' || UPPER(SUBSTRING(v_row.item_id::text FROM 1 FOR 8)), NULL,
            'Pénzforgalmi ÁFA átvezetés (47993->467) - ' || COALESCE(v_matched_inv.invoice_number, ''),
            'HUF', 1.0, v_date, 'PF-VAT-TR-' || v_row.item_id::text
  ...
  FOR v_matched_tr IN
    ...
    LEFT JOIN public.nav_invoices ni ON ni.company_id = p_company_id AND (ni.transaction_id = t.id OR ni.id = t.matched_invoice_id)
    LEFT JOIN public.invoices inv ON inv.company_id = p_company_id AND (inv.transaction_id = t.id OR inv.id = t.matched_invoice_id)
    WHERE t.company_id = p_company_id
      AND (t.matched_invoice_id IS NOT NULL OR ni.id IS NOT NULL OR inv.id IS NOT NULL)
  ...
  LOOP
    ...
    -- Pénzforgalmi ÁFA átvezetés Vegyes naplóban (VE)
    IF v_matched_tr.vat_amount > 0 AND COALESCE(v_matched_tr.payment_method, '') <> 'CASH' AND v_journal_ve_id IS NOT NULL THEN
      ...
      IF v_matched_tr.invoice_direction = 'OUTBOUND' AND v_is_penzforgalmi THEN
        INSERT INTO public.acc_journal_headers (
          ...
          p_company_id, v_journal_ve_id, EXTRACT(YEAR FROM v_date)::SMALLINT, 'GEPI_JAVASLAT', 'NORMAL', 'AUTO_SZAMLA',
          v_date, v_date, 'AFA-' || UPPER(SUBSTRING(v_matched_tr.transaction_id::text FROM 1 FOR 8)), v_partner_id,
          'Pénzforgalmi ÁFA átvezetés (47993->467) - ' || COALESCE(v_matched_tr.invoice_number, ''),
          'HUF', 1.0, v_date, 'PF-VAT-TR-' || v_matched_tr.transaction_id::text
        ) RETURNING id INTO v_ve_header_id;
