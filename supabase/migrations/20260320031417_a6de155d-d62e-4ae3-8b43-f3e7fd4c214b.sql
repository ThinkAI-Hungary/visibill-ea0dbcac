...
-- Phase 5a: Server-side filtered NAV invoices with pagination
...
-- Phase 5b: Server-side filtered submitted invoices with pagination
...
CREATE INDEX IF NOT EXISTS idx_nav_invoices_company_direction_date 
  ON nav_invoices(company_id, invoice_direction, invoice_issue_date);

CREATE INDEX IF NOT EXISTS idx_invoices_company_direction_date 
  ON invoices(company_id, invoice_direction, kibocsatas_datuma);
