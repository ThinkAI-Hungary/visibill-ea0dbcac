-- Migration: 20260903110000_accounting_journals_daily_exchange_rates.sql
-- Description: Integration with daily_exchange_rates table for MNB daily exchange rate on posting date
-- Ticket / Request: "Az összeg ha nem HUF currencyben -van akkor ()ben az adott napi árfolyammal egyezően meg kell jeleníteni"
-- Author: Visibill Dev
-- Date: 2026-09-03
...
DECLARE
  ...
  v_gl_cust_id UUID;
  v_gl_supp_id UUID;
  v_gl_vat_ded_id UUID; -- 466 Levonható ÁFA
  v_gl_vat_pay_id UUID; -- 467 Fizetendő ÁFA
  v_count INTEGER := 0;
  v_date DATE;
  v_amount NUMERIC;
...
BEGIN
  ...
  -- 6. Loop over classified ledger items
  FOR v_row IN 
  ...
  LOOP
    ...
    -- Resolve currency, foreign amount, and lookup daily MNB exchange rate on or before posting date
    ...
    IF v_currency <> 'HUF' THEN
      -- 1. Look up historical daily exchange rate on or before item date
      SELECT rate, rate_date INTO v_exchange_rate, v_exchange_rate_date
      ...
      -- 2. Fallback to closest available rate if no prior rate exists
      ...
      -- 3. Default fallback if currency not found in table
      IF v_exchange_rate IS NULL OR v_exchange_rate <= 0 THEN
        v_exchange_rate := 1.000000;
        v_exchange_rate_date := v_date;
      END IF;

      -- HUF amount is calculated from foreign amount using daily exchange rate
    ...
    ELSIF v_row.source_table IN ('invoice_items', 'nav_invoice_items') THEN
      ...
      -- Fallback for direction if neither set
      IF v_final_direction IS NULL THEN
        IF v_row.amount >= 0 THEN
          v_final_direction := 'OUTBOUND';
        ELSE
          v_final_direction := 'INBOUND';
        END IF;
      END IF;

      -- Determine if this is a credit note / negative adjustment
      IF (v_item_net IS NOT NULL AND v_item_net < 0)
         OR (v_final_direction = 'OUTBOUND' AND v_row.original_amount IS NOT NULL AND v_row.original_amount < 0)
         OR (v_final_direction = 'INBOUND' AND v_row.original_amount IS NOT NULL AND v_row.original_amount > 0) THEN
        v_is_credit := TRUE;
      ELSE
        v_is_credit := FALSE;
      END IF;

      -- Calculate 3-legged amounts:
      -- 1. Net in HUF and Foreign
      v_huf_net := ROUND(ABS(v_amount), 2);
      
      IF v_currency <> 'HUF' THEN
        v_foreign_net := ROUND(ABS(COALESCE(v_item_net, v_amount_foreign)), 2);
      ELSE
        v_foreign_net := NULL;
      END IF;

      -- 2. VAT in HUF and Foreign
      IF v_final_direction = 'OUTBOUND' AND v_gl_vat_pay_id IS NULL THEN
        v_item_vat := 0;
      ELSIF v_final_direction = 'INBOUND' AND v_gl_vat_ded_id IS NULL THEN
        v_item_vat := 0;
      END IF;

      IF v_item_vat IS NOT NULL AND ABS(v_item_vat) > 0 THEN
        IF v_currency <> 'HUF' AND v_exchange_rate <> 1.000000 THEN
          v_foreign_vat := ROUND(ABS(v_item_vat), 2);
          v_huf_vat := ROUND(v_foreign_vat * v_exchange_rate, 2);
        ELSE
          v_foreign_vat := NULL;
          v_huf_vat := ROUND(ABS(v_item_vat), 2);
        END IF;
      ELSE
        v_foreign_vat := NULL;
        v_huf_vat := 0;
      ...
      -- 3. Gross amount (Net + VAT = Gross)
      v_huf_gross := v_huf_net + COALESCE(v_huf_vat, 0);
      IF v_currency <> 'HUF' THEN
        v_foreign_gross := v_foreign_net + COALESCE(v_foreign_vat, 0);
      ELSE
      ...
      IF v_final_direction = 'OUTBOUND' THEN
        ...
        IF NOT v_is_credit THEN
          ...
          END IF;
        ELSE
          -- Credit note: T Revenue (Nettó ALAP), T VAT 467 (AFA), K Customer 311 (Bruttó)
          INSERT INTO public.acc_journal_lines (
            header_id, sequence_number, gl_account_id, dc_type, amount, foreign_amount, vat_code, vat_role, description
          ) VALUES (
      ...
      ELSE
        ...
        INSERT INTO public.acc_journal_headers (
          ...
          p_company_id, v_journal_id, EXTRACT(YEAR FROM v_date)::SMALLINT, 'GEPI_JAVASLAT', 'NORMAL', 'AUTO_SZAMLA',
          v_date, v_date, v_doc_id, v_partner_id,
          COALESCE(v_partner_name, 'Szállító') || ' - ' || COALESCE(v_row.description, 'Beszerzés'), v_currency, v_exchange_rate, v_exchange_rate_date, v_row.item_id::text,
          v_gl_cls, (v_gl_cls ->> 'confidence_score')::numeric
        ) RETURNING id INTO v_header_id;
  ...
  RETURN v_count;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' SET statement_timeout = '60s';

-- Explicit grants
GRANT EXECUTE ON FUNCTION public.acc_generate_drafts_from_ledger(UUID, UUID) TO authenticated, anon, service_role;
GRANT EXECUTE ON FUNCTION public.acc_seed_default_journals(UUID) TO authenticated, anon, service_role;
GRANT EXECUTE ON FUNCTION public.get_gl_categorized_items(UUID, UUID, date, date, jsonb, text) TO authenticated, anon, service_role;

-- Retroactive fix for existing unposted foreign currency headers and lines:
-- 1. Update exchange_rate and exchange_rate_date from daily_exchange_rates
UPDATE public.acc_journal_headers h
SET exchange_rate = r.rate,
    exchange_rate_date = r.rate_date
FROM (
  SELECT DISTINCT ON (h2.id) h2.id, dr.rate, dr.rate_date
  FROM public.acc_journal_headers h2
  JOIN public.daily_exchange_rates dr 
    ON dr.currency = h2.currency 
   AND dr.rate_date <= h2.posting_date
  WHERE h2.currency <> 'HUF'
    AND h2.status <> 'KONYVELT'
  ORDER BY h2.id, dr.rate_date DESC
) r
WHERE h.id = r.id;

-- 2. Update acc_journal_lines amount to foreign_amount * exchange_rate for unposted foreign headers
UPDATE public.acc_journal_lines l
SET amount = ROUND(l.foreign_amount * h.exchange_rate, 2)
FROM public.acc_journal_headers h
WHERE l.header_id = h.id 
  AND h.currency <> 'HUF' 
  AND h.status <> 'KONYVELT'
  AND l.foreign_amount IS NOT NULL 
  AND h.exchange_rate IS NOT NULL 
  AND h.exchange_rate > 1;
