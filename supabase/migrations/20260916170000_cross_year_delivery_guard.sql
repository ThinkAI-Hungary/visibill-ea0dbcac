-- Migration: 20260916170000_cross_year_delivery_guard.sql
-- Description:
--   1. Add is_cross_year column to invoices and nav_invoices tables.
--   2. Add check_cross_year_delivery triggers on invoices and nav_invoices.
--   3. Backfill existing cross-year invoices.
--   4. Update acc_generate_drafts_from_ledger to skip generating current-year P&L drafts
--      for cross-year invoices (where delivery date is in a prior year).

-- ─── 1. ADD COLUMNS & INDEXES ──────────────────────────────────────────────────
ALTER TABLE public.invoices 
  ADD COLUMN IF NOT EXISTS is_cross_year BOOLEAN NOT NULL DEFAULT false;

ALTER TABLE public.nav_invoices 
  ADD COLUMN IF NOT EXISTS is_cross_year BOOLEAN NOT NULL DEFAULT false;

CREATE INDEX IF NOT EXISTS idx_invoices_cross_year 
  ON public.invoices(company_id) WHERE is_cross_year = true;

CREATE INDEX IF NOT EXISTS idx_nav_invoices_cross_year 
  ON public.nav_invoices(company_id) WHERE is_cross_year = true;

-- ─── 2. TRIGGERS FOR AUTO-MARKING CROSS-YEAR INVOICES ──────────────────────────
CREATE OR REPLACE FUNCTION public.check_cross_year_delivery()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF TG_TABLE_NAME = 'invoices' THEN
    IF NEW.teljesites_datuma IS NOT NULL AND NEW.kibocsatas_datuma IS NOT NULL 
       AND EXTRACT(YEAR FROM NEW.teljesites_datuma) < EXTRACT(YEAR FROM NEW.kibocsatas_datuma) THEN
      NEW.is_cross_year := true;
    ELSE
      NEW.is_cross_year := false;
    END IF;
  ELSIF TG_TABLE_NAME = 'nav_invoices' THEN
    IF NEW.invoice_delivery_date IS NOT NULL AND NEW.invoice_issue_date IS NOT NULL 
       AND EXTRACT(YEAR FROM NEW.invoice_delivery_date) < EXTRACT(YEAR FROM NEW.invoice_issue_date) THEN
      NEW.is_cross_year := true;
    ELSE
      NEW.is_cross_year := false;
    END IF;
...
DROP TRIGGER IF EXISTS trg_check_cross_year_invoices ON public.invoices;
CREATE TRIGGER trg_check_cross_year_invoices
  BEFORE INSERT OR UPDATE OF teljesites_datuma, kibocsatas_datuma ON public.invoices
  FOR EACH ROW
  EXECUTE FUNCTION public.check_cross_year_delivery();

DROP TRIGGER IF EXISTS trg_check_cross_year_nav_invoices ON public.nav_invoices;
CREATE TRIGGER trg_check_cross_year_nav_invoices
  BEFORE INSERT OR UPDATE OF invoice_delivery_date, invoice_issue_date ON public.nav_invoices
  FOR EACH ROW
  EXECUTE FUNCTION public.check_cross_year_delivery();

-- ─── 3. BACKFILL EXISTING RECORDS ──────────────────────────────────────────────
UPDATE public.invoices
   SET is_cross_year = true
 WHERE teljesites_datuma IS NOT NULL AND kibocsatas_datuma IS NOT NULL
   AND EXTRACT(YEAR FROM teljesites_datuma) < EXTRACT(YEAR FROM kibocsatas_datuma);

UPDATE public.nav_invoices
   SET is_cross_year = true
 WHERE invoice_delivery_date IS NOT NULL AND invoice_issue_date IS NOT NULL
   AND EXTRACT(YEAR FROM invoice_delivery_date) < EXTRACT(YEAR FROM invoice_issue_date);

-- ─── 4. RECREATE ACC_GENERATE_DRAFTS_FROM_LEDGER WITH CROSS-YEAR GUARD ─────────
