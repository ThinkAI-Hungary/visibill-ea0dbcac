-- =============================================================================
-- Migration: Refactor get_management_files RPC to Pure CTE (Eliminate Temp Tables)
-- Description:
--   Refactors public.get_management_files(...) from procedural CREATE/DROP TEMP TABLE
--   statements to pure Common Table Expressions (CTEs).
--
-- Rationale:
--   1. Migration 20260925110000_mark_all_read_rpcs_stable_and_harden_rate_limiter.sql
--      correctly marked read-only analytical/reporting RPCs as STABLE.
--   2. However, PostgreSQL strictly disallows DDL (DROP TABLE, CREATE TEMP TABLE)
--      inside non-volatile functions (error 0A000: DROP TABLE is not allowed in a non-volatile function).
--   3. Converting the raw multi-table union and window ranking into CTEs eliminates
--      all DDL, prevents transaction pooler / connection catalog contention,
--      ensures true STABLE semantics, and significantly boosts query performance.
--
-- Related ADR: A-068, A-088, A-152
-- =============================================================================
