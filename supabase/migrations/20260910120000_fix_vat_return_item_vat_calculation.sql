-- Migration: 20260910120000_fix_vat_return_item_vat_calculation.sql
-- Description: Fallback tax calculation for nav_invoice_items when vat_amount is 0 or null

-- 1. Update nav_invoice_items for existing items where vat_amount is 0
UPDATE nav_invoice_items
SET vat_amount = ROUND(net_amount * 0.27, 2)
WHERE (vat_amount IS NULL OR vat_amount = 0)
  AND net_amount > 0
  AND vat_rate IN ('0.27', '27', '27.0', '27.00', '27%');

UPDATE nav_invoice_items
SET vat_amount = ROUND(net_amount * 0.18, 2)
WHERE (vat_amount IS NULL OR vat_amount = 0)
  AND net_amount > 0
  AND vat_rate IN ('0.18', '18', '18.0', '18.00', '18%');

UPDATE nav_invoice_items
SET vat_amount = ROUND(net_amount * 0.05, 2)
WHERE (vat_amount IS NULL OR vat_amount = 0)
  AND net_amount > 0
  AND vat_rate IN ('0.05', '5', '5.0', '5.00', '5%');

-- 2. Update calculate_vat_return function to include tax calculation fallback
