-- Migration: Hardening Accounting Drafts Generator & Subledger Opening Reconciliation
-- Date: 2026-09-03
-- Purpose:
-- 1. Eliminate circular draft generation from acc_journal_lines in acc_generate_drafts_from_ledger
-- 2. Clean up any invalid drafts created from acc_journal_lines
-- 3. Enhance acc_check_opening_subledger_reconciliation to aggregate all NY entries and handle historical payment dates safely

-- ─── 1. CLEAN UP INVALID DRAFTS CREATED FROM INTERNAL JOURNAL LINES ─────────

DELETE FROM public.acc_journal_headers 
WHERE status = 'GEPI_JAVASLAT' 
  AND source = 'AUTO_RENDSZER' 
  AND EXISTS (SELECT 1 FROM public.acc_journal_lines l WHERE l.id::text = import_key);


-- ─── 2. RECREATE ACC_CHECK_OPENING_SUBLEDGER_RECONCILIATION (HARDENED) ──────
...
BEGIN
  -- 1. Nyitott kimenő (Vevő) számlák összege a számla analitikából
  -- Szűrés: a nyitó év előtti bizonylatok, amelyek a nyitás napján (p_year-01-01) kifizetetlenek voltak
  SELECT COALESCE(SUM(COALESCE(brutto_vegosszeg, adoalap_osszesen + afa_osszeg_osszesen, 0)), 0)
   ...
   WHERE company_id = p_company_id
     ...
     AND COALESCE(kibocsatas_datuma, teljesites_datuma) < MAKE_DATE(p_year, 1, 1)
     AND (
       fizetve IS FALSE 
       OR fizetve IS NULL 
       OR (
         -- Ha már ki van fizetve, de a fizetés a nyitó évben vagy később történt (tehát nyitáskor még nyitott volt):
         EXISTS (
           SELECT 1 FROM public.transactions t 
           WHERE t.id = invoices.transaction_id 
             AND t.transaction_date >= MAKE_DATE(p_year, 1, 1)
         )
         OR (manual_payment_date >= MAKE_DATE(p_year, 1, 1))
       )
     );

  -- 2. Nyitott bejövő (Szállító) számlák összege a számla analitikából
  -- Szűrés: a nyitó év előtti bizonylatok, amelyek a nyitás napján (p_year-01-01) kifizetetlenek voltak
  SELECT COALESCE(SUM(COALESCE(brutto_vegosszeg, adoalap_osszesen + afa_osszeg_osszesen, 0)), 0)
    INTO v_open_ap_sum
    FROM public.invoices
  ...
  -- 3. Főkönyvi 311-es és 454-es nyitó egyenlegek az összes lekönyvelt NY bizonylatból (összesítve)
...
-- ─── 3. RECREATE ACC_GENERATE_DRAFTS_FROM_LEDGER (CIRCULAR DEFENSE) ─────────
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_row.source_table IN ('invoice_items', 'nav_invoice_items') THEN
      ...
      -- Partner resolution
      v_partner_id := NULL;
      IF v_partner_tax IS NOT NULL AND v_partner_tax <> '' THEN
        SELECT id INTO v_partner_id FROM public.partners 
         WHERE company_id = p_company_id AND tax_number = v_partner_tax LIMIT 1;
      END IF;
      IF v_partner_id IS NULL AND v_partner_name IS NOT NULL AND v_partner_name <> '' THEN
        SELECT id INTO v_partner_id FROM public.partners 
         WHERE company_id = p_company_id AND name = v_partner_name LIMIT 1;
      END IF;
