-- ==============================================================================
-- Migration: 20260914170000_fix_vat_return_item_fallback_and_rlb_gl.sql
-- Description:
--   1. calculate_vat_return: Use LEFT JOIN nav_invoice_items with fallback to
--      header amounts (invoice_net_amount, invoice_vat_amount) when details_fetched=false.
--   2. acc_generate_drafts_from_ledger:
--      - Resolve payable VAT: Prioritize 4667% (e.g. 46671 'Fizetendő általános forgalmi adó'
--        in RLB-60 imported chart of accounts) when 467 is 'Pénzforgalmi fizetendő áfa'.
--      - Resolve deductible VAT: Exclude 4667% from 466% matching.
--      - Resolve PF payable: 47993 / 47911 / 467 (if named pénzforgalmi).
--   3. Data fix: Update existing VBV journal lines from 467 to 46671.
--   4. Recalculate 2026-01 VAT return for VBV Vision Kft.
-- ==============================================================================
...
BEGIN
  ...
  -- NOTE: Uses LEFT JOIN with fallback to invoice_net_amount / invoice_vat_amount if items not yet fetched
  FOR inv_rec IN
    SELECT 
      ni.id AS invoice_id,
...
-- 2. Update acc_generate_drafts_from_ledger with RLB-60 VAT account handling
...
-- 3. Data fix: Update existing VBV journal lines from 467 (cd98ca1c-4f09-4d5d-9aea-c11f71406abb)
-- to 46671 (6c9084b2-3c9c-4983-afeb-f1f61aa1aee8) using visibill.allow_gl_remap session config
DO $$
BEGIN
  PERFORM set_config('visibill.allow_gl_remap', 'true', true);
  UPDATE public.acc_journal_lines
     SET gl_account_id = '6c9084b2-3c9c-4983-afeb-f1f61aa1aee8'
   WHERE gl_account_id = 'cd98ca1c-4f09-4d5d-9aea-c11f71406abb';
END $$;


-- 4. Recalculate 2026-01 VAT return for VBV Vision Kft.
SELECT public.calculate_vat_return('5364d0be-e92a-4b94-9704-f457cf71f140', 2026, 1, 'H');
