-- ============================================================
-- FIX: ÁFA BEVALLÁS — Frequency szűrés az upsert-ben
-- ============================================================
-- A calculate_vat_return RPC nem szűrt frequency-re, így a 
-- havi és negyedéves bevallás ugyanazt a rekordot találta meg.
-- Ez a migráció az EREDETI RPC-t tartja meg, csak a frequency 
-- szűrést javítja a SELECT és a prev_carry lekérdezésben.
...
BEGIN
  ...
  -- 4. Get previous period carryforward — FIX: ALSO FILTER ON FREQUENCY
  ...
  SELECT
    ...
    -- Aggregate per INVOICE (not per item) to avoid duplicate invoice numbers
    (SELECT jsonb_agg(inv_summary ORDER BY inv_summary->>'delivery_date')
     FROM (
       SELECT jsonb_build_object(
         ...
         'items', (
           SELECT jsonb_agg(jsonb_build_object(
             'description', nii3.line_description,
             'net', nii3.net_amount,
             'vat', nii3.vat_amount,
             'vat_rate', nii3.vat_rate,
             'quantity', nii3.quantity,
             'unit_price', nii3.unit_price
           ) ORDER BY nii3.line_number)
           FROM nav_invoice_items nii3
           WHERE nii3.nav_invoice_id = ni2.id
             AND nii3.vat_rate NOT IN ('0','0.0','0.00','TAM','AAM')
         )
       ) AS inv_summary
       ...
       WHERE ni2.company_id = p_company_id
         ...
         AND nii2.vat_rate NOT IN ('0','0.0','0.00','TAM','AAM')
         AND ni2.supplier_tax_number = ni.supplier_tax_number
       GROUP BY ni2.id, ni2.invoice_number, ni2.invoice_delivery_date
     ) sub
    )
  FROM nav_invoice_items nii
  JOIN nav_invoices ni ON ni.id = nii.nav_invoice_id
