-- Migration: Fix Balance Sheet double counting and sign issues for bank, receivables, and payables
  ...
  aggregated AS (
    SELECT
      ...
      CASE
        ...
        WHEN s.id = (SELECT mapped_bs_id FROM bank_target) THEN (SELECT bank_total FROM auto_bank)
        WHEN s.id = (SELECT mapped_bs_id FROM receivables_target) THEN
          (SELECT receivables_total FROM auto_receivables) - COALESCE(SUM(md.total_balance), 0)::numeric
        WHEN s.id = (SELECT mapped_bs_id FROM payables_target) THEN
          -((SELECT payables_total FROM auto_payables) + COALESCE(SUM(md.total_balance), 0)::numeric)
