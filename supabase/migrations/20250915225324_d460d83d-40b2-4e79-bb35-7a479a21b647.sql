...
-- First, get the project IDs for our user
WITH project_ids AS (
  SELECT 
    id,
    name,
...
SET project_id = CASE 
  ...
  WHEN szamlaszam IN ('AAA-2025-15', 'BOSZE-2025-39') THEN 
    (SELECT id FROM project_ids WHERE name = 'Oktatás és Képzés')
  WHEN szamlaszam IN ('E-TXFYB-2025-11', 'E-TXFYB-2025-21') THEN 
    (SELECT id FROM project_ids WHERE name = 'Személyi Szolgáltatások')
  WHEN szamlaszam = 'C20253232' THEN 
    (SELECT id FROM project_ids WHERE name = 'Építőipar és Szolgáltatások')
  WHEN szamlaszam = 'E-VK-2025-220' THEN 
    (SELECT id FROM project_ids WHERE name = 'Marketing és Reklám')
  WHEN szamlaszam = 'KM-2025-12' THEN 
    (SELECT id FROM project_ids WHERE name = 'Irodai és Adminisztratív')
  ELSE NULL
END
WHERE user_id = 'e5b822ee-4240-4350-9ebe-a14357d5bd89';