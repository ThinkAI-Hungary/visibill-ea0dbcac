-- Migration: 20260825010000_fix_vat_return_calculation_and_codes.sql
-- Description: Fix calculate_vat_return to auto-seed vat_codes and use COALESCE date fallback for robust VAT calculation.

-- 1. Populate missing invoice_delivery_date in nav_invoices with invoice_issue_date fallback
UPDATE nav_invoices 
SET invoice_delivery_date = COALESCE(invoice_delivery_date, invoice_issue_date)
WHERE invoice_delivery_date IS NULL AND invoice_issue_date IS NOT NULL;

-- 2. Seed default vat codes for all existing companies that don't have them yet
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN SELECT id FROM companies WHERE id NOT IN (SELECT DISTINCT company_id FROM vat_codes)
  LOOP
...
-- 3. Update calculate_vat_return RPC with auto-seed and robust date resolution
...
BEGIN
  ...
  --    WITH PÉNZFORGALMI LOGIC & ROBUST DATE RESOLUTION
  FOR v_code IN
  ...
  LOOP
    ...
    ELSE
      -- ═══════════════════════════════════════════════════════
      -- NORMÁL ESET — with pénzforgalmi branching & robust date
      -- ═══════════════════════════════════════════════════════
      SELECT
        COALESCE(SUM(nii.net_amount), 0),
