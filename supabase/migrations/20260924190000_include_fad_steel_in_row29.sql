-- Migration: 20260924190000_include_fad_steel_in_row29.sql
-- Description: Ensure domestic reverse-charge steel products (Áfa tv. 142. § (1) i) / 6/B. melléklet, KN 72/73)
--              and FAD purchases are correctly mapped to Row 29 (Fizetendő fordított adó) and Row 66 (Levonható adó / 66_fad),
--              calculating the mandatory 27% self-assessed VAT on inbound transactions.

-- 1. Backfill is_reverse_charge on nav_invoices that contain domestic reverse charge items (steel / FAD) with 0 invoice VAT
UPDATE nav_invoices ni
SET is_reverse_charge = true
WHERE ni.is_reverse_charge = false
  ...
  AND EXISTS (
    ...
    WHERE nii.nav_invoice_id = ni.id
      AND (
        COALESCE(nii.product_code, '') ~ '^(72|73)'
        OR COALESCE(nii.line_description, '') ~* '^(72|73)[0-9]{2}'
        OR COALESCE(nii.line_description, '') ~* '(acél|betonacél|zártszelvény|idomacél|gerenda|lemez|háló|fémhulladék)'
        OR COALESCE(nii.vat_rate, '') ILIKE '%FAD%'
        OR COALESCE(nii.net_weight_kg, 0) > 0
      )
  );

-- 2. Update calculate_vat_return function
...
BEGIN
  ...
  LOOP
    ...
    IF v_effective_tax_date BETWEEN v_date_from AND v_date_to THEN
      ...
      -- Determine whether THIS SPECIFIC LINE ITEM is domestic reverse charge (FAD / steel products under Áfa tv. 6/B)
      -- Reverse charge items MUST have 0 supplier tax (Áfa tv. 142. §)
      v_is_fad := (
