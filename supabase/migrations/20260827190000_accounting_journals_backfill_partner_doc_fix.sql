-- Migration: Fix Journals Backfill Partner and Document Numbers
-- Date: 2026-08-27
  ...
  LOOP
    ...
    ELSIF v_row.source_table IN ('invoice_items', 'nav_invoice_items') THEN
      ...
      ELSE
        ...
        -- Lookup partner by tax number or name
        IF v_partner_tax IS NOT NULL THEN
          SELECT id INTO v_partner_id FROM public.partners 
           WHERE company_id = p_company_id 
             AND (tax_number LIKE v_partner_tax || '%' OR vat_number LIKE v_partner_tax || '%') 
           LIMIT 1;
        END IF;
