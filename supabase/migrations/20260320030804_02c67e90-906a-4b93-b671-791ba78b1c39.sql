...
-- RPC to compute petty cash balance server-side (replaces 5 parallel unbounded client queries)
...
BEGIN
  ...
  SELECT COALESCE(SUM(ABS(ni.invoice_gross_amount)), 0) INTO v_cash_sales
  ...
  SELECT COALESCE(SUM(ABS(i.brutto_vegosszeg)), 0) INTO v_cash_expenses
  FROM invoices i
  ...
  SELECT COALESCE(SUM(ABS(ni.invoice_gross_amount)), 0) INTO v_nav_cash_expenses
  FROM nav_invoices ni
  WHERE ni.company_id = p_company_id
