-- Migration: 20260903100000_accounting_journals_3leg_vat_drafts.sql
-- Description: 3-legged double-entry posting for accounting journals (Net + VAT = Gross)
-- Ticket: EB-0045 & Audit fixes (Credit note support, missing VAT account fallback)
...
BEGIN
  ...
  SELECT id INTO v_gl_supp_id 
    FROM public.gl_accounts 
   WHERE preset_id = p_preset_id AND gl_number LIKE '4541%' 
   ORDER BY gl_number LIMIT 1;
  ...
  LOOP
    ...
    ELSIF v_row.source_table IN ('invoice_items', 'nav_invoice_items') THEN
      ...
      -- 3. VAT in HUF and Foreign
      -- Guard: If VAT account cannot be found, fallback to 0 VAT to preserve balance
      ...
      -- 4. Gross strictly = Net + VAT (guarantees zero penny imbalance)
      v_huf_gross := v_huf_net + v_huf_vat;
      IF v_currency <> 'HUF' THEN
        v_foreign_gross := ROUND(v_foreign_net + COALESCE(v_foreign_vat, 0), 2);
      ELSE
