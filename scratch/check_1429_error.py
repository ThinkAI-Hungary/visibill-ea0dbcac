...
res = supabase.table("app_error_logs").select("*").eq("severity", "warning").order("created_at", desc=True).limit(10).execute()

for r in res.data:
    print(f"[{r['created_at']}] [{r['severity']}] {r['component']} -> {r['action']}")
