...
res = supabase.table("invoice_uploads").select("*").gte("created_at", "2026-09-01T13:00:00Z").order("created_at", desc=True).execute()

print(f"Found {len(res.data)} upload(s) since 15:00:")
for u in res.data:
    subject = u.get("metadata", {}).get("subject", "")
    sender = u.get("metadata", {}).get("sender", "")
    source = u.get("metadata", {}).get("source", "")
    print(f"[{u['created_at']}] {u['file_name']} | Status: {u['upload_status']}/{u['processing_status']} | Source: {source} | Subject: {subject}")
