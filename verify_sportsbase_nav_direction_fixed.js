...
async function check() {
  ...
  const directions = {};
  counts.forEach(row => {
    directions[row.invoice_direction] = (directions[row.invoice_direction] || 0) + 1;
  });
  ...
  const { data: outbounds } = await supabase
    .from('nav_invoices')
    .select('invoice_number, invoice_direction, invoice_gross_amount, invoice_issue_date')
    .eq('company_id', companyId)
