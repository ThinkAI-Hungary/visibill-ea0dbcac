...
const IosToggle = React.forwardRef<HTMLButtonElement, IosToggleProps>(({
  ...
  return <button ref={ref} role="switch" aria-checked={checked} aria-label={ariaLabel || (checked ? onLabel : offLabel)} disabled={disabled} onClick={handleClick} onKeyDown={handleKeyDown} className={cn(
  ...
  // Disabled state
  disabled && "cursor-not-allowed opacity-50", className)} {...props}>
        {/* Label text - only show the label on the opposite side of the knob */}
        {checked ? <span className="absolute left-[4px] top-1/2 -translate-y-1/2 text-[8px] font-bold uppercase tracking-tight text-success-foreground mx-[4px]">
            {onLabel}
          </span> : <span className="absolute right-[4px] top-1/2 -translate-y-1/2 text-[8px] font-bold uppercase tracking-tight text-warning-foreground mx-[4px]">
            {offLabel}
          </span>}
