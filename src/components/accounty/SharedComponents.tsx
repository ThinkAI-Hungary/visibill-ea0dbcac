...
export function KpiCard({ title, value, subtitle, icon: Icon, valueClass = 'text-foreground', accentColor = 'teal' }: KpiCardProps) {
  return (
    <div
      className={cn(
        "relative overflow-hidden bg-gradient-to-br rounded-lg p-5 border border-border shadow-soft flex flex-col justify-between h-32 card-ripple",
        " hover:border-border dark:hover:border-border transition-all duration-300 cursor-default group",
...
export function SkeletonKpiCard() {
  return (
    <div className="bg-card rounded-lg p-5 border border-border h-32 animate-pulse">
...
export function SkeletonListRow() {
  return (
    <div className="bg-card rounded-lg p-4 border border-border animate-pulse h-20 flex items-center gap-3">
...
export function SkeletonClientCard({ index = 0 }: { index?: number }) {
  return (
    <div className="bg-card rounded-lg border border-border overflow-hidden animate-pulse" style={{ animationDelay: `${index * 100}ms` }}>
...
export function EmptyState({ icon: Icon, title, description, action }: EmptyStateProps) {
  return (
    <div className="py-16 text-center page-animate">
      ...
      {items.map((item, i) => (
        <React.Fragment key={i}>
          ...
          {item.href ? (
            ...
            <span className="text-foreground/90 font-semibold">{item.label}</span>
          )}
        </React.Fragment>
      ))}
    </nav>
