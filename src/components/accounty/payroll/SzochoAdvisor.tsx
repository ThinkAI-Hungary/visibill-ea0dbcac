...
export function SzochoAdvisor({ companyId }: SzochoAdvisorProps) {
  ...
  if (employeesLoading || employmentsLoading) {
    return (
      <div className="p-6 bg-background/10 rounded-lg border border-border animate-pulse">
  ...
  if (recommendations.length === 0) {
    return (
      <div className="p-5 bg-emerald-50/50 dark:bg-emerald-950/10 rounded-lg border border-emerald-100/60 dark:border-emerald-900/30 flex items-start gap-4 page-animate">
        <div className="p-2 bg-emerald-100 dark:bg-emerald-900/30 rounded-lg text-emerald-600 shrink-0">
          <UserCheck className="w-5 h-5" />
        </div>
        <div>
          <h3 className="text-sm font-bold text-foreground">SZOCHO optimalizáció</h3>
  ...
  if (pendingRecommendations.length === 0) {
    return (
      <div className="bg-gradient-to-br from-emerald-50/40 via-teal-50/20 to-card dark:from-emerald-950/10 dark:via-teal-950/5 dark:to-card border border-emerald-100 dark:border-emerald-950 rounded-lg p-5 shadow-soft flex items-start justify-between flex-wrap gap-4 page-animate">
        <div className="flex items-start gap-4">
          <div className="p-2.5 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-lg shadow-lg shadow-emerald-500/20 text-white shrink-0">
            <UserCheck className="w-5 h-5" />
          </div>
          <div>
            <h3 className="text-sm font-bold text-foreground">Minden kedvezmény érvényesítve</h3>
            <p className="text-xs text-muted-foreground mt-0.5">
  ...
  return (
    <div className="bg-gradient-to-br from-indigo-50/40 via-purple-50/30 to-card dark:from-indigo-950/10 dark:via-purple-950/5 dark:to-card border border-indigo-100 dark:border-indigo-950 rounded-lg p-5 shadow-soft space-y-4 page-animate">
      {/* Advisor Header */}
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg shadow-lg shadow-primary/20 text-white">
            <Sparkles className="w-5 h-5" />
          </div>
          <div>
            <h2 className="text-base font-bold text-foreground">AI Bérszámfejtési SZOCHO Tanácsadó</h2>
            <p className="text-xs text-muted-foreground">Automatikus kedvezmény-detektálás FEOR és életkor alapján</p>
          </div>
        </div>

        <div className="flex gap-4">
          {totalPotentialSavings > 0 && (
            <div className="bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-lg text-right">
          ...
          <div key={rec.employmentId} className="py-3 first:pt-0 last:pb-0 flex items-center justify-between gap-4">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-sm font-bold text-foreground">{rec.employeeName}</span>
                <span className="text-xs text-muted-foreground">({rec.jobTitle} - FEOR {rec.feorCode || '–'})</span>
              </div>
              <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1">
                <ShieldAlert className="w-3.5 h-3.5 text-primary" />
                Javasolt kedvezmény: <strong className="text-primary dark:text-primary">{rec.eligibleLabel}</strong>
              </p>
            </div>
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  className="h-8 text-xs bg-primary hover:bg-primary/90 text-primary-foreground hover:text-white border-none font-semibold px-4 rounded-lg"
