    ...
    <Card className="bg-orange-500/[0.06] border-orange-400/40 expand-animate">
      <CardHeader className="py-2 px-3">
        <CardTitle className="text-xs font-medium flex items-center justify-between">
          <span className="flex items-center gap-1.5">
            ...
            {t('invoices:expanded_netting.title', 'Kompenzálási javaslat')}
          </span>
      ...
      <CardContent className="p-3 pt-0 space-y-2">
        <div className="text-xs">
          <span className="text-muted-foreground">{t('invoices:expanded_netting.partner', 'Partner:')}</span>
        ...
        <div className="space-y-1.5">
          {nettingGroup.inboundInvoices.length > 0 && (
            <div>
              <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-1">
                {t('invoices:expanded_netting.inbound_title', { count: nettingGroup.inboundInvoices.length, defaultValue: `Bejövő számlák (${nettingGroup.inboundInvoices.length})` })}
              </div>
              {nettingGroup.inboundInvoices.map((inv) => (
                ...
                >
                  ...
                  <span className="font-mono text-destructive">
                    {formatCurrencyLocale(Math.abs(inv.invoice_gross_amount || 0), inv.currency || 'HUF')}
                  </span>
                </div>
              ))}
            </div>
          ...
          {nettingGroup.outboundInvoices.length > 0 && (
            <div>
              <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-1">
                {t('invoices:expanded_netting.outbound_title', { count: nettingGroup.outboundInvoices.length, defaultValue: `Kimenő számlák (${nettingGroup.outboundInvoices.length})` })}
              </div>
        ...
        <div className="border-t border-orange-400/20 pt-2 grid grid-cols-3 gap-2 text-xs">
          <div>
            <span className="text-muted-foreground">{t('invoices:expanded_netting.inbound_total', 'Bejövő összesen:')}</span>
            <div className="font-mono font-medium text-destructive">
              {formatCurrencyLocale(nettingGroup.inboundTotal, nettingGroup.currency)}
            </div>
          </div>
          <div>
            <span className="text-muted-foreground">{t('invoices:expanded_netting.outbound_total', 'Kimenő összesen:')}</span>
            <div className="font-mono font-medium text-success">
              {formatCurrencyLocale(nettingGroup.outboundTotal, nettingGroup.currency)}
            </div>
          </div>
          <div>
            <span className="text-muted-foreground">{t('invoices:expanded_netting.net_diff', 'Nettó különbözet:')}</span>
            <div
              className={cn(
                "font-mono font-bold",
            ...
            >
              {formatCurrencyLocale(Math.abs(nettingGroup.netDifference), nettingGroup.currency)}
              <span className="text-[10px] font-normal text-muted-foreground ml-1">
                ({nettingGroup.netDifference >= 0 ? t('invoices:expanded_netting.receivable', 'követelés') : t('invoices:expanded_netting.payable', 'tartozás')})
              </span>
            </div>
          </div>
        </div>
