    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-card rounded-xl border border-border shadow-soft overflow-hidden">
        ...
        <div className="p-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { num: '1', title: 'Ügyfél hozzáadása', desc: 'Portfólió → Új ügyfél → Adószám megadása' },
              { num: '2', title: 'Hiányok áttekintése', desc: 'Hiányzó számlák → Cég kiválasztása → Részletek' },
              { num: '3', title: 'Bekérés indítása', desc: 'AI Hívás vagy Magic Link küldése az ügyfélnek' },
            ].map(step => (
              <div key={step.num} className="flex items-start gap-3 p-4 rounded-lg bg-primary/5 border border-primary/10">
                <div className="w-8 h-8 rounded-full bg-primary/15 flex items-center justify-center shrink-0 text-sm font-bold text-primary">{step.num}</div>
                <div>
      ...
      <div className="bg-card rounded-xl border border-border shadow-soft overflow-hidden">
        ...
        <div className="divide-y divide-border">
          ...
          {searchActive && filteredFaqs.length === 0 && (
...
export function HelpCategoriesSection({ filteredCategories, searchActive }: CategoriesProps) {
            ...
            <div className={cn(
              ...
              <div className="px-5 pb-5 space-y-2 border-t border-border pt-3">
                {cat.articles.map((article, i) => (
                  <div key={i} className="p-3 rounded-lg hover:bg-accent/50 transition-colors cursor-pointer group">
...
export function HelpVideosSection({ filteredVideos, searchActive }: VideosProps) {
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      {searchActive && filteredVideos.length === 0 && (
        <p className="p-8 text-center text-sm text-muted-foreground">Nincs találat a keresésre.</p>
      )}
...
export function HelpLegislationSection({ filteredLegislations, searchActive }: LegislationProps) {
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      {searchActive && filteredLegislations.length === 0 && (
        <p className="p-8 text-center text-sm text-muted-foreground">Nincs találat a keresésre.</p>
      )}
    </div>
  );
...
export function HelpGlossarySection({ filteredGlossary, searchActive }: GlossaryProps) {
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      {searchActive && filteredGlossary.length === 0 && (
        <p className="p-8 text-center text-sm text-muted-foreground">Nincs találat a keresésre.</p>
      )}
    </div>
  );
