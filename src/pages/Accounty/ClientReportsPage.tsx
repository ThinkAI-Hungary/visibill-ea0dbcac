...
export default function ClientReportsPage() {
  ...
  const openModal = (type: ReportType) => {
    if (type === 'hianyzo') {
      navigate(`/eaisybooks/client/${id}/reports/missing-invoices`);
      return;
    }
  ...
  return (
    <div className="w-full space-y-8 page-animate relative pb-24">
      {/* Header */}
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <h1 className="text-2xl font-bold text-foreground tracking-tight">Riportok</h1>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-card border border-border rounded-lg shadow-soft overflow-hidden">
          <div className="p-12 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">A generált riportok itt fognak megjelenni</p>
          </div>
        </div>
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
          ...
          <div className="relative bg-card rounded-lg shadow-xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 z-10 flex flex-col max-h-[90vh]">
            {/* Modal Header */}
            <div className="flex justify-between items-start p-6 border-b border-border shrink-0">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">Állítsd be a {clientName} riport paramétereit</p>
              </div>
              <button 
                onClick={() => setIsModalOpen(false)}
                className="text-muted-foreground hover:text-muted-foreground dark:hover:text-muted-foreground/60 dark:text-muted-foreground p-1 rounded-full hover:bg-muted dark:bg-muted transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    <Checkbox id="opt-1" defaultChecked className="border-border rounded" />
            ...
            <div className="p-4 border-t border-border flex items-center justify-between shrink-0 dark:bg-card/50">
              <Button variant="ghost" className="text-muted-foreground hover:text-foreground/90 dark:hover:text-foreground dark:text-foreground/90 whitespace-nowrap" onClick={() => setIsModalOpen(false)}>
                Mégse
              </Button>
              <div className="flex gap-2">
                <Button variant="outline" className="gap-2 bg-card text-foreground/90 border-border whitespace-nowrap">
