    ...
    <div className="w-full space-y-6 page-animate">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">Jogviszonykódok</h1>
            <p className="text-sm text-muted-foreground">NAV jogviszonykódok master tábla</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex gap-1 bg-muted/50 rounded-lg p-0.5">
          {([['all', 'Mind'], ['active', 'Aktív'], ['inactive', 'Inaktív']] as [Filter, string][]).map(([v, l]) => (
            <button key={v} onClick={() => setFilter(v)} className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${filter === v ? 'bg-card dark:bg-muted text-foreground shadow-sm' : 'text-muted-foreground'}`}>
              {l}
            </button>
          ))}
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border dark:bg-card/30">
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Kód</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Megnevezés</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Biztosítás</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Min. járulékalap</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Érvényesség</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Aktív</th>
          ...
          <tbody className="divide-y divide-border/50">
            {isLoading ? (
              ...
              paginated.map((c: any) => (
                <tr key={c.id || c.code} className="hover:bg-muted/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${c.is_insured ? 'bg-green-50 text-green-600 dark:bg-green-900/30 dark:text-green-400' : 'bg-muted text-muted-foreground dark:bg-muted dark:text-muted-foreground'}`}>
                      {c.is_insured ? 'Igen' : 'Nem'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-xs text-muted-foreground">{c.min_contribution_base_rule || '-'}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className={`w-2.5 h-2.5 rounded-full ${c.is_active ? 'bg-green-500' : 'bg-muted-foreground/30'}`} />
                  </td>
        ...
        <DialogContent className="sm:max-w-lg bg-card">
          ...
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm font-medium text-foreground/90">Kód</label>
                <Input value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} placeholder="pl. 1101" className="mt-1 font-mono" />
              </div>
              <div>
                <label className="text-sm font-medium text-foreground/90">Megnevezés</label>
                <Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="mt-1" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm font-medium text-foreground/90">Érvényes tól</label>
                <DatePicker
                  value={form.valid_from}
                  onChange={dateStr => setForm(f => ({ ...f, valid_from: dateStr }))}
              ...
              <div>
                <label className="text-sm font-medium text-foreground/90">Érvényes ig</label>
                <DatePicker
                  value={form.valid_to}
                  onChange={dateStr => setForm(f => ({ ...f, valid_to: dateStr }))}
                  placeholder="éééé. hh. nn."
                  clearable
                  className="mt-1"
                />
            ...
            <div>
              <label className="text-sm font-medium text-foreground/90">Min. járulékalap szabály</label>
            ...
            <div className="flex items-center gap-6">
              <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
                <Checkbox
                  ...
                  onCheckedChange={checked => setForm(f => ({ ...f, is_insured: Boolean(checked) }))}
                />
                Biztosított
              </label>
              <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
                <Checkbox
                  ...
                  onCheckedChange={checked => setForm(f => ({ ...f, is_active: Boolean(checked) }))}
                />
                Aktív
              </label>
            </div>
