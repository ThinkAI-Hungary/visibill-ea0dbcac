...
export default function TaoTaxpayerTypesPage() {
  ...
  const REGIME_COLORS: Record<string, string> = {
    ...
    'Nonprofit (B)': 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-primary',
  };
  ...
  return (
    <div className="w-full space-y-6 page-animate">
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground tracking-tight">Adózói Körök</h1>
          <p className="text-sm text-muted-foreground mt-1">Portfólió bontása GFO-kód és adózói típus szerint</p>
        </div>
      </div>

      {/* Visual breakdown - horizontal bar */}
      <div className="bg-card rounded-lg border border-border p-6 shadow-soft">
        <h2 className="text-sm font-bold text-foreground/90 mb-4">Adózói típus megoszlása</h2>
        <div className="flex h-6 rounded-full overflow-hidden bg-muted">
          {types.filter(t => t.count > 0).map((t, i) => {
            const pct = (t.count / total) * 100;
            const colors = ['bg-emerald-500', 'bg-blue-500', 'bg-amber-500', 'bg-purple-500', 'bg-rose-500', 'bg-cyan-500', 'bg-orange-500', 'bg-primary'];
            return (
              <div
                key={t.gfo}
          ...
          {types.filter(t => t.count > 0).map((t, i) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[10px] text-muted-foreground font-medium">{t.name.split('(')[0].trim()} ({t.count})</span>
              </button>
            );
          })}
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border dark:bg-card/30">
                <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">GFO-kód</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Adózói típus</th>
                <th className="text-center px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Ügyfélszám</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Adóalap-rezsim</th>
              ...
              {types.map(t => (
                ...
                >
                  <td className="px-4 py-3">
                    <span className="text-sm font-mono font-bold text-foreground/90">{t.gfo}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-sm font-medium text-foreground">{t.name}</span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span className={cn('text-sm font-bold', t.count > 0 ? 'text-foreground/90' : 'text-muted-foreground/60')}>{t.count}</span>
                  </td>
                  <td className="px-4 py-3">
            ...
            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
              {selectedGfoClients.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">Nincs hozzárendelt cég ebben az adózói körben.</p>
                </div>
              ) : (
                selectedGfoClients.map(c => (
                  <div
                    key={c.id}
                    className="flex items-center justify-between p-3 rounded-lg border border-border bg-muted/40/50 dark:bg-muted/30 hover:bg-muted/50 transition-colors"
                  >
