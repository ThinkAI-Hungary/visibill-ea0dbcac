  ...
  return (
    <div className="w-full space-y-6 page-animate max-w-3xl">
      <div className="flex items-center gap-3">
        ...
        <div className="p-2.5 bg-gradient-to-br from-green-500 to-emerald-600 rounded-lg shadow-lg shadow-green-500/25">
          <Banknote className="w-5 h-5 text-white" />
        </div>
        <div>
          <h1 className="text-2xl font-bold text-foreground">Devizában Fizetés</h1>
          <p className="text-sm text-muted-foreground">TAO USD/EUR-ben fizetésének választása (spec 2.7)</p>
        </div>
      </div>

      <div className="bg-card rounded-lg border border-border p-6 shadow-soft space-y-5">
        <h2 className="text-base font-bold text-foreground">Pénznem választó</h2>
        <div className="flex gap-4">
          {(['HUF', 'USD', 'EUR'] as const).map(c => (
            <label key={c} className={cn(
              'flex-1 p-5 rounded-lg border-2 cursor-pointer transition-all text-center',
        ...
        {currency !== 'HUF' && (
          <div className="space-y-4 mt-4">
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Kincstári deviza-számlaszám</label>
