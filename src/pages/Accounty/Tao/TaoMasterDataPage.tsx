  ...
  return (
    <div className="w-full space-y-6 page-animate max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">TAO Törzsadatok</h1>
          <p className="text-sm text-muted-foreground">{client?.name || 'Ügyfél'}</p>
        </div>
      </div>

      {/* Alapadatok */}
      <div className="bg-card rounded-lg border border-border p-6 shadow-soft space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Cégnév</label>
            <Input value={formData.companyName} onChange={e => update('companyName', e.target.value)} className="bg-background" />
          </div>
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Adószám (xx-x-yy)</label>
            <Input value={formData.taxNumber} onChange={e => update('taxNumber', e.target.value)} className="bg-background font-mono" />
          </div>
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Cégjegyzékszám</label>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">GFO-kód</label>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Adóalap-rezsim</label>
            <select
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">KKV-besorolás</label>
            <select
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">Adóalanyiság kezdete</label>
            <DatePicker
              value={formData.taxSubjectStart}
              onChange={val => update('taxSubjectStart', val)}
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <Checkbox
              ...
              onCheckedChange={checked => update('groupMember', !!checked)}
            />
            <span className="text-sm text-foreground/90">Csoportos TAO-alany tagja (2/A.§)</span>
          </label>
        </div>
        {formData.groupMember && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Csoport-azonosító</label>
              <Input value={formData.groupId} onChange={e => update('groupId', e.target.value)} className="bg-background" />
            </div>
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Képviselő</label>
              <Input value={formData.groupRepresentative} onChange={e => update('groupRepresentative', e.target.value)} className="bg-background" />
            </div>
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Szavazati arány (≥75%)</label>
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <Checkbox
              ...
              onCheckedChange={checked => update('pillarTwo', !!checked)}
            />
            <span className="text-sm text-foreground/90">Multinacionális csoport-tag (750 M EUR küszöb)</span>
          </label>
        </div>
        {formData.pillarTwo && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Konszolidált bevétel (4 év)</label>
            ...
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">Végső anyavállalat</label>
