...
const STATUS_BADGE: Record<string, { label: string; color: string }> = {
  ...
  expired: { label: 'Felváltva', color: 'bg-muted text-muted-foreground dark:bg-muted dark:text-muted-foreground' },
  archived: { label: 'Archivált', color: 'bg-muted text-muted-foreground dark:bg-muted dark:text-muted-foreground' },
    ...
    <div className="w-full max-w-5xl mx-auto space-y-6 page-animate">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Korábbi és érvényes nyilatkozatok teljes előzménye</p>
          </div>
        </div>
        <ExportButton
      ...
      {isLoading ? (
        ...
        <div className="bg-card rounded-lg border border-border p-12 text-center space-y-3">
          ...
          <p className="text-sm text-muted-foreground">Nincsenek archivált nyilatkozatok.</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-4 gap-3">
            <div className="bg-card rounded-lg border border-border p-4 text-center"><p className="text-2xl font-bold text-emerald-600">{declList.filter(d => d.status === 'active').length}</p><p className="text-xs text-muted-foreground">Érvényes</p></div>
            <div className="bg-card rounded-lg border border-border p-4 text-center"><p className="text-2xl font-bold text-muted-foreground">{declList.filter(d => d.status === 'expired').length}</p><p className="text-xs text-muted-foreground">Lejárt</p></div>
            <div className="bg-card rounded-lg border border-border p-4 text-center"><p className="text-2xl font-bold text-amber-500">{declList.filter(d => (d.status as string) === 'archived').length}</p><p className="text-xs text-muted-foreground">Archivált</p></div>
            <div className="bg-card rounded-lg border border-border p-4 text-center"><p className="text-2xl font-bold">{declList.length}</p><p className="text-xs text-muted-foreground">Összesen</p></div>
          </div>
          ...
          <div className="bg-card rounded-lg border border-border shadow-soft overflow-hidden">
            <div className="px-5 py-3 border-b border-border dark:bg-card/30">
              <h2 className="text-sm font-bold text-foreground/90">Archivált nyilatkozatok ({filtered.length})</h2>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left px-5 py-2 text-xs font-bold text-muted-foreground">Munkavállaló</th>
                  <th className="text-left px-3 py-2 text-xs font-bold text-muted-foreground">Típus</th>
                  <th className="text-center px-3 py-2 text-xs font-bold text-muted-foreground">Év</th>
                  <th className="text-center px-3 py-2 text-xs font-bold text-muted-foreground">Státusz</th>
                  <th className="text-center px-3 py-2 text-xs font-bold text-muted-foreground">Beadva</th>
                  <th className="px-3 py-2" />
                </tr>
              </thead>
              <tbody>
                {paginated.map(dec => (
                  <tr key={dec.id} className={cn('border-b border-border/50 hover:bg-muted/50 transition-colors', dec.status !== 'active' && 'opacity-60')}>
                    <td className="px-5 py-2.5 font-medium">{String(dec.data?.employeeName || '—')}</td>
                    <td className="px-3 py-2.5"><span className={cn('px-2 py-0.5 rounded-full text-[10px] font-bold', TYPE_COLORS[dec.type] || 'bg-muted text-muted-foreground')}>{TYPE_LABELS[dec.type] || dec.type}</span></td>
                    ...
                    <td className="px-3 py-2.5 text-center text-xs text-muted-foreground">{dec.filedAt ? new Date(dec.filedAt).toLocaleDateString('hu-HU') : '—'}</td>
            ...
            {filtered.length === 0 && <div className="px-5 py-8 text-center text-sm text-muted-foreground">Nincs találat a szűrési feltételeknek.</div>}
          </div>
        </>
      )}
    </div>
