    ...
    <div className="w-full space-y-6 page-animate">
      ...
      {!hideHeader && (
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-gradient-to-br from-red-500 to-rose-600 rounded-lg shadow-lg shadow-red-500/25">
            <Clock className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-foreground">NAV határidők</h1>
            <p className="text-sm text-muted-foreground">Bejelentési, bevallási és befizetési naptár</p>
          </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-card rounded-lg border border-border p-4 shadow-soft">
          <p className="text-xs text-muted-foreground mb-1">Aktív határidők</p>
          <p className="text-2xl font-bold text-foreground">{active}</p>
        </div>
        <div className={cn('rounded-lg border p-4', dueToday > 0 ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800' : 'bg-card border-border shadow-soft')}>
        ...
        <div className={cn('rounded-lg border p-4', overdue > 0 ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800' : 'bg-card border-border shadow-soft')}>
              ...
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ...
                    : 'bg-muted/50 text-muted-foreground border-transparent hover:border-border dark:hover:border-border'
                )}
              >
                <div className={cn(
                  'w-2 h-2 rounded-full transition-all',
                  isActive ? conf.bg + ' border ' + conf.dotBorder : 'bg-muted-foreground/30'
                )} />
                {conf.label}
                <span className={cn(
      ...
      {grouped.length === 0 ? (
        <div className="bg-card rounded-lg border border-border p-12 text-center shadow-soft">
          ...
          <p className="text-sm text-muted-foreground">Nincs találat a szűrőkkel</p>
        </div>
      ) : (
        grouped.map(([monthKey, items]) => {
          ...
          return (
            <div key={monthKey} className="space-y-3">
              <h2 className="text-sm font-bold text-foreground/90 uppercase tracking-wider">{monthName}</h2>
              {items.map(deadline => {
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className={cn(
                        ...
                        <span className="text-[9px] text-muted-foreground uppercase">{new Date(parseInt(y), parseInt(m) - 1).toLocaleDateString('hu-HU', { month: 'short' })}</span>
                      </div>
                      {/* Content */}
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <Icon className={cn('w-3.5 h-3.5', conf.color)} />
                          <h3 className={cn('text-sm font-bold', isCompleted ? 'text-green-700 dark:text-green-400 line-through' : 'text-foreground')}>
                            {deadline.title}
                          </h3>
                          {isDueToday && (
                      ...
                      {/* Date */}
                      <span className="text-xs text-muted-foreground font-mono shrink-0">{deadline.date}</span>
                    </div>
                  </div>
                );
              })}
