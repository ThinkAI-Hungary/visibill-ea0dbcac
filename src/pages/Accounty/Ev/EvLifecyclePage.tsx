  ...
  const handleSaveEvent = async () => {
    ...
    try {
      const insertData = {
        ...
        from_form: newEvent.eventType === 'form_change' ? (newEvent.fromForm as any || null) : null,
        to_form: newEvent.eventType === 'form_change' ? (newEvent.toForm as any || null) : null,
      };
      const { error } = await supabase.from('accounty_ev_lifecycle_events').insert(insertData as any);
  ...
  return (
    <div className="w-full space-y-6 page-animate">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <ChevronRight className="w-3 h-3" />
        <span className="text-foreground font-medium">Életciklus</span>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">EV Életciklus</h1>
            <p className="text-sm text-muted-foreground">{client?.name || 'Ügyfél'} – tevékenység története, adóforma-változások</p>
          </div>
        </div>
        <button
      ...
      {/* Add event form */}
      {showAddForm && (
        <div className="bg-card rounded-lg border-2 border-indigo-200 dark:border-indigo-800 shadow-soft p-5 space-y-4 animate-in slide-in-from-top-2 duration-300">
          <h3 className="text-sm font-bold text-foreground">Új életciklus esemény rögzítése</h3>
              ...
              <button
                ...
                className={cn(
                  ...
                  newEvent.eventType === type
                    ? 'border-primary bg-primary/10/50 dark:bg-indigo-900/20'
                    : 'border-border hover:border-border'
                )}
              >
                <span className={config.color}>{config.icon}</span>
                <span className="text-[11px] font-medium text-foreground/90">{config.label}</span>
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted-foreground">Esemény dátuma</label>
              <DatePicker
                value={newEvent.eventDate}
                onChange={val => setNewEvent(f => ({ ...f, eventDate: val }))}
            ...
            {newEvent.eventType === 'form_change' && (
              <>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-muted-foreground">Eredeti forma</label>
                  <select
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-muted-foreground">Új forma</label>
                  <select
            ...
            <div className={cn('space-y-1.5', newEvent.eventType !== 'form_change' && 'col-span-1')}>
              <label className="text-xs font-medium text-muted-foreground">Megjegyzés (opcionális)</label>
              <Input
          ...
          <div className="flex justify-end">
            <button
              ...
              className="flex items-center gap-1.5 px-5 py-2 text-sm font-medium bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors disabled:opacity-50"
            >
      ...
      {isLoading ? (
        ...
        <div className="bg-card rounded-lg border border-border shadow-soft p-12 text-center">
          <GitBranch className="w-10 h-10 mx-auto mb-3 text-muted-foreground/60" />
          <p className="text-sm text-muted-foreground">Nincs még életciklus esemény rögzítve.</p>
          <p className="text-xs text-muted-foreground mt-1">Kattintson az „Új esemény" gombra az első esemény rögzítéséhez.</p>
        </div>
      ) : events.length > 0 && (
        <>
          {/* Current status card */}
          <div className="bg-card rounded-lg border border-border shadow-soft p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium mb-2">Jelenlegi státusz</p>
                <div className="flex items-center gap-3">
                  <span className={cn(
                  ...
                  <span className="text-sm text-foreground/90">
                    Adóforma: <strong className="text-primary">{FORM_LABELS[currentForm] || currentForm}</strong>
                  </span>
                </div>
              </div>
              {firstEvent && (
                <div className="text-right">
                  <p className="text-xs text-muted-foreground">Tevékenység kezdete</p>
            ...
            {[...events].reverse().map((event, idx, arr) => {
              ...
              return (
                <div key={event.id} className="flex items-stretch">
                  ...
                  <div className={cn(
                    'flex-1 ml-4 bg-card rounded-lg border border-border shadow-soft p-4',
                    isLast ? 'mb-0' : 'mb-4',
                      ...
                      <div>
                        ...
                        {event.eventType === 'form_change' && event.fromForm && event.toForm && (
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-xs font-medium px-2 py-0.5 rounded bg-indigo-100 dark:bg-indigo-900/30 text-primary">
