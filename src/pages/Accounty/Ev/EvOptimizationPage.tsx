...
export default function EvOptimizationPage() {
  ...
  const FORM_COLORS: Record<string, string> = {
    atalany: 'from-blue-500 to-primary',
  ...
  return (
    <div className="w-full space-y-6 page-animate">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <ChevronRight className="w-3 h-3" />
        <span className="text-foreground font-medium">Adóoptimalizálás</span>
      </div>

      {/* Header */}
      <div className="flex items-center gap-3">
        <div className="p-2.5 bg-gradient-to-br from-yellow-500 to-amber-600 rounded-lg shadow-lg shadow-yellow-500/25">
          <Lightbulb className="w-5 h-5 text-white" />
        </div>
        <div>
          <h1 className="text-2xl font-bold text-foreground">Adóoptimalizálás</h1>
          <p className="text-sm text-muted-foreground">AI-támogatott javaslatok az adóteher csökkentéséhez</p>
        </div>
      </div>
      ...
      <div className="bg-card rounded-lg border border-border shadow-soft p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Éves bevétel (Ft)</label>
            <input
              type="number"
              value={revenue}
              onChange={e => setRevenue(+e.target.value)}
              className="w-full px-3 py-2 border border-border rounded-lg bg-background text-sm font-mono tabular-nums text-foreground"
            />
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Ténylegesen felmerült költségek (Ft)</label>
            <input
              type="number"
              value={costs}
          ...
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Vállalkozói kivét (Ft)</label>
            <input
              type="number"
              value={kivet}
      ...
      {/* Comparison */}
      <div>
        <h2 className="text-sm font-bold text-foreground mb-3">Adóforma összehasonlítás</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {comparisons.map(c => (
            <div key={c.form} className={cn(
              'rounded-lg border-2 shadow-soft overflow-hidden transition-all',
              ...
              <div className="p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-base font-bold text-foreground">{c.label}</h3>
                </div>
                <p className="text-2xl font-bold font-mono tabular-nums text-foreground mb-1">
      ...
      {/* Savings highlight */}
      {maxSaving > 0 && (
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-lg border border-green-200 dark:border-green-800 p-5">
          <div className="flex items-center gap-3">
        ...
        <div className="space-y-3">
          {suggestions.map((tip, i) => (
            <div key={i} className={cn(
              'bg-card rounded-lg border shadow-soft px-5 py-4 flex items-start gap-4',
              ...
              {tip.saving > 0 && (
                <div className="text-right flex-shrink-0">
                  ...
                  <p className="text-[10px] text-muted-foreground">megtakarítás/év</p>
                </div>
              )}
            </div>
          ))}
