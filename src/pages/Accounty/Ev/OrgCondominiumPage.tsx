...
const FUND_COLORS: Record<string, { bg: string; bar: string; text: string }> = {
  ...
  egyeb: { bg: 'bg-muted', bar: 'bg-muted-foreground', text: 'text-muted-foreground' },
};

const STATUS_CFG: Record<string, { label: string; color: string; icon: React.ElementType }> = {
  planned: { label: 'Tervezett', color: 'bg-muted text-muted-foreground dark:bg-muted dark:text-muted-foreground', icon: Clock },
  ...
  return (
    <div className="w-full space-y-6 page-animate">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span className="text-foreground font-medium">Társasház</span>
      </div>

      {/* Header */}
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-gradient-to-br from-sky-500 to-blue-600 rounded-lg shadow-lg shadow-sky-500/25">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <div className="bg-card rounded-lg border border-border p-3 space-y-1">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Albetétek</p>
            <p className="text-lg font-bold text-foreground">{activeUnitsCount}</p>
          </div>
          <div className="bg-card rounded-lg border border-border p-3 space-y-1">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Havi közös ktg.</p>
            <p className="text-lg font-bold text-green-600 font-mono tabular-nums">{formatHuf(totalMonthlyFee)}</p>
          </div>
          <div className="bg-card rounded-lg border border-border p-3 space-y-1">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Hátralékok</p>
            <p className={cn('text-lg font-bold font-mono tabular-nums', totalArrears > 0 ? 'text-red-600' : 'text-muted-foreground')}>
              {totalArrears > 0 ? formatHuf(totalArrears) : '0 Ft'}
            </p>
          </div>
          <div className="bg-card rounded-lg border border-border p-3 space-y-1">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Alapok egyenleg</p>
          ...
          <div className="bg-card rounded-lg border border-border p-3 space-y-1">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Karb. terv ktg.</p>
      ...
      {/* ═══ OVERVIEW TAB ═══ */}
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 page-animate">
          {/* Funds overview */}
                  ...
                  return (
                    <div key={fund.id} className="px-4 py-3">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-xs font-semibold text-foreground/90">{fund.fund_name}</span>
            ...
            <div className="divide-y divide-border/50">
              {arrearsCount === 0 ? (
                <div className="px-4 py-6 text-center">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1">Minden lakó rendben fizetett</p>
                </div>
              ) : (
                (units || []).filter(u => u.arrears_amount > 0).slice(0, 5).map(u => (
                  <div key={u.id} className="flex items-center justify-between px-4 py-2.5 text-xs">
                    <div>
                      <p className="font-semibold text-foreground/90">{u.unit_number} — {u.owner_name}</p>
                      {u.last_payment_date && <p className="text-[10px] text-muted-foreground">Utolsó fiz.: {new Date(u.last_payment_date).toLocaleDateString('hu-HU')}</p>}
                    </div>
          ...
          {/* Active maintenance */}
          <div className="md:col-span-2 bg-card rounded-lg border border-border shadow-soft overflow-hidden">
            <div className="px-4 py-3 border-b border-border flex items-center justify-between">
              <h3 className="text-xs font-bold text-foreground uppercase tracking-wider flex items-center gap-1.5">
            ...
            <div className="divide-y divide-border/50">
              {(!maintenance || maintenance.filter(m => m.status !== 'completed' && m.status !== 'cancelled').length === 0) ? (
                <div className="px-4 py-6 text-center text-xs text-muted-foreground">Nincs aktív feladat</div>
              ) : (
                maintenance.filter(m => m.status !== 'completed' && m.status !== 'cancelled').slice(0, 4).map(m => {
                  ...
                  return (
                    <div key={m.id} className="flex items-center justify-between px-4 py-2.5 text-xs">
                      ...
                      <span className="font-mono tabular-nums text-muted-foreground">{m.estimated_cost > 0 ? formatHuf(m.estimated_cost) : '–'}</span>
                    </div>
                  );
                })
              )}
      ...
      {activeTab === 'funds' && (
        <div className="space-y-4 page-animate">
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">{funds?.length || 0} pénzalap</p>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(funds || []).map(fund => {
              ...
              const pct = fund.target_balance > 0 ? Math.min(100, (fund.current_balance / fund.target_balance) * 100) : 0;
              return (
                <div key={fund.id} className={cn('rounded-lg border border-border p-5 space-y-3', colors.bg)}>
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-sm font-bold text-foreground">{fund.fund_name}</h3>
                      {fund.description && <p className="text-[10px] text-muted-foreground mt-0.5">{fund.description}</p>}
                    </div>
                    <button
                      onClick={() => setFundModal({ open: true, edit: fund })}
                      className="p-1.5 rounded-lg hover:bg-white/50 dark:hover:bg-muted/50 transition-colors"
                    >
                  ...
                  <div className="flex items-end justify-between">
                    <div>
                      <p className="text-[10px] text-muted-foreground">Aktuális egyenleg</p>
                      <p className={cn('text-2xl font-bold font-mono tabular-nums', colors.text)}>{formatHuf(fund.current_balance)}</p>
                    </div>
                    {fund.target_balance > 0 && (
                      <div className="text-right">
                        <p className="text-[10px] text-muted-foreground">Cél</p>
                        <p className="text-sm font-mono tabular-nums text-muted-foreground">{formatHuf(fund.target_balance)}</p>
                      </div>
                    )}
                  </div>
                  ...
                  {fund.target_balance > 0 && (
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[10px] text-muted-foreground">Töltöttség</span>
                        <span className={cn('text-[10px] font-bold', colors.text)}>{Math.round(pct)}%</span>
                      </div>
                      <div className="w-full h-2 bg-white/50 dark:bg-muted rounded-full overflow-hidden">
          ...
          {(!funds || funds.length === 0) && (
            <div className="bg-card rounded-lg border border-dashed border-border p-8 text-center">
              ...
              <p className="text-sm font-medium text-muted-foreground">Még nincs pénzalap</p>
              <p className="text-xs text-muted-foreground mt-1">Add hozzá az üzemeltetési, felújítási és tartalék alapokat</p>
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === 'debtors' && (
        <div className="space-y-4 page-animate">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <p className="text-sm text-muted-foreground">{activeUnitsCount} aktív albetét</p>
              {arrearsCount > 0 && (
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border bg-muted/40/50 dark:bg-muted/30">
                    <th className="text-left py-2.5 px-4 font-medium text-muted-foreground uppercase tracking-wider">Albetét</th>
                    <th className="text-left py-2.5 px-4 font-medium text-muted-foreground uppercase tracking-wider">Típus</th>
                    <th className="text-left py-2.5 px-4 font-medium text-muted-foreground uppercase tracking-wider">Tulajdonos</th>
                    <th className="text-right py-2.5 px-4 font-medium text-muted-foreground uppercase tracking-wider">Terület</th>
                    <th className="text-right py-2.5 px-4 font-medium text-muted-foreground uppercase tracking-wider">Havi díj</th>
                    <th className="text-right py-2.5 px-4 font-medium text-muted-foreground uppercase tracking-wider">Hátralék</th>
                    <th className="text-center py-2.5 px-4 font-medium text-muted-foreground uppercase tracking-wider">Státusz</th>
                  ...
                  {(!units || units.length === 0) ? (
                    <tr>
                      <td colSpan={8} className="py-12 text-center text-sm text-muted-foreground">
                        <Building2 className="w-8 h-8 mx-auto mb-3 text-muted-foreground/60" />
                    ...
                    units.map(u => {
                      ...
                      return (
                        <tr key={u.id} className={cn(
                          'hover:bg-muted/50 dark:hover:bg-muted/50/30 transition-colors',
                          ...
                          <td className="py-2.5 px-4 text-muted-foreground">{UNIT_TYPE_LABELS[u.unit_type] || u.unit_type}</td>
                          <td className="py-2.5 px-4">
                            <p className="font-medium text-foreground/90">{u.owner_name}</p>
                            {u.owner_contact && <p className="text-[10px] text-muted-foreground flex items-center gap-1"><Phone className="w-2.5 h-2.5" />{u.owner_contact}</p>}
                          </td>
                          <td className="py-2.5 px-4 text-right font-mono tabular-nums text-muted-foreground">{u.area_sqm ? `${u.area_sqm} m²` : '–'}</td>
                          <td className="py-2.5 px-4 text-right font-mono tabular-nums text-foreground/90">{formatHuf(u.monthly_common_fee)}</td>
                          <td className="py-2.5 px-4 text-right font-mono tabular-nums">
                            <span className={hasArrears ? 'text-red-600 font-bold' : 'text-muted-foreground'}>
                ...
                {units && units.length > 0 && (
                  <tfoot>
                    <tr className="border-t-2 border-border bg-muted/40/30 dark:bg-muted/20 font-bold text-xs">
                      <td colSpan={4} className="py-2.5 px-4 text-foreground">Összesen ({activeUnitsCount} albetét)</td>
                      <td className="py-2.5 px-4 text-right font-mono tabular-nums text-foreground">{formatHuf(totalMonthlyFee)}</td>
      ...
      {activeTab === 'maintenance' && (
        <div className="space-y-4 page-animate">
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">{maintenance?.length || 0} feladat</p>
            <button
          ...
          <div className="space-y-2">
            {(!maintenance || maintenance.length === 0) ? (
              <div className="bg-card rounded-lg border border-dashed border-border p-8 text-center">
                <Wrench className="w-8 h-8 mx-auto mb-3 text-muted-foreground/60" />
                <p className="text-sm font-medium text-muted-foreground">Még nincs karbantartási feladat</p>
                <p className="text-xs text-muted-foreground mt-1">Rögzítse a tervezett és folyamatban lévő munkákat</p>
              </div>
            ) : (
              maintenance.map(m => {
                const cfg = STATUS_CFG[m.status] || STATUS_CFG.planned;
                ...
                return (
                  <div key={m.id} className={cn(
                    'bg-card rounded-lg border shadow-soft px-5 py-4 flex items-start justify-between gap-4',
                    ...
                    <div className="flex-1 min-w-0 space-y-1.5">
                      ...
                      <div className="flex items-center gap-4 text-[10px] text-muted-foreground">
                        {m.vendor_name && <span>Kivitelező: <span className="font-medium text-muted-foreground">{m.vendor_name}</span></span>}
                      ...
                      <div className="text-right">
                        {m.estimated_cost > 0 && (
                          <p className="text-xs font-mono tabular-nums text-muted-foreground">
                            {m.actual_cost > 0 ? (
                              <><span className="text-foreground font-bold">{formatHuf(m.actual_cost)}</span> / {formatHuf(m.estimated_cost)}</>
