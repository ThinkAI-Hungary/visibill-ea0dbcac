      ...
      <DialogContent className="sm:max-w-[600px] p-0 gap-0 overflow-hidden">
        <>
          <div className="px-6 py-4 flex items-center justify-between border-b border-border">
            <DialogTitle className="text-xl font-bold text-foreground">{invoice.subtext}</DialogTitle>
          ...
          <div className="px-6 py-5 grid grid-cols-2 gap-y-6 gap-x-4">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-foreground">{invoice.createdAt ? new Date(invoice.createdAt).toLocaleDateString('hu-HU') : '-'}</p>
            </div>
          </div>
          ...
          <div className="px-6 py-5 border-t border-border">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm font-medium text-foreground">{invoice.navInvoiceId || '-'}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground mb-1">Számla szám</p>
          ...
          <div className="px-6 py-5 border-t border-border">
            ...
            {invoice.notificationCount > 0 ? (
                  ...
                  <div className="flex items-center justify-between p-3 bg-card border border-border rounded-lg shadow-soft">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Email – {invoice.notificationCount}. bekérés</p>
                      </div>
                    </div>
              ...
              <p className="text-sm text-muted-foreground italic">Még nem lett bekérve.</p>
            )}
          </div>
          ...
          {invoice.uploadedFiles.length > 0 && (
            <div className="px-6 py-5 border-t border-border">
              <h3 className="text-sm font-bold text-foreground mb-3">Feltöltött dokumentumok</h3>
              <div className="space-y-2">
                {invoice.uploadedFiles.map((filePath, idx) => {
                  const fileName = filePath.split('/').pop() || filePath;
                  const displayName = fileName.replace(/^\d+_/, '');
                  return (
                    <div key={idx} className="flex items-center justify-between p-3 bg-card border border-border rounded-lg shadow-soft">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] text-muted-foreground">Portálon keresztül feltöltve</p>
                        </div>
                      </div>
