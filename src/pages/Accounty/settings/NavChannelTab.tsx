  ...
  return (
    <div key="nav" className="p-6 space-y-6 tab-content-enter">
      <div className="border-b border-border pb-4">
        <h2 className="text-lg font-bold text-foreground">NAV csatorna beállítások</h2>
        <p className="text-sm text-muted-foreground mt-1">Online Számla API, bevallás-beküldés és ÁNYK integráció</p>
      </div>

      <div>
        <h3 className="text-sm font-bold text-foreground mb-3">NAV Online Számla API</h3>
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <label className="text-xs text-muted-foreground">Technikai felhasználó</label>
            <Input value={navTechnicalUser} onChange={e => setNavTechnicalUser(e.target.value)} className="bg-card border-border font-mono" />
          </div>
          <div className="space-y-2">
            <label className="text-xs text-muted-foreground">Környezet</label>
          ...
          <div className="space-y-2">
            <label className="text-xs text-muted-foreground">API kulcs</label>
            <Input type="password" value={navApiKey} onChange={e => setNavApiKey(e.target.value)} className="bg-card border-border font-mono" />
          </div>
          <div className="space-y-2">
            <label className="text-xs text-muted-foreground">Aláíró kulcs (XML signature)</label>
      ...
      <div>
        <h3 className="text-sm font-bold text-foreground mb-3">Bevallás-beküldés mód</h3>
        <div className="space-y-3">
          <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-foreground">KR (Kormányzati API)</p>
                <p className="text-xs text-muted-foreground">Közvetlen NAV beküldés XML-ben</p>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-foreground">ÁNYK (Általános Nyomtatványkitöltő)</p>
                <p className="text-xs text-muted-foreground">Offline kitöltő programmal — ABEV export</p>
              </div>
            </div>
      ...
      <div className="space-y-2">
        <label className="text-xs text-muted-foreground">ÁNYK telepítési útvonal</label>
        <Input value={navAnykPath} onChange={e => setNavAnykPath(e.target.value)} className="bg-card border-border font-mono text-xs" />
        <p className="text-[10px] text-muted-foreground">Csak ÁNYK módú beküldésnél szükséges</p>
      </div>

      <label className="flex items-center gap-3 p-4 bg-muted/50 rounded-lg cursor-pointer select-none">
        <Checkbox checked={navAutoSubmit} onCheckedChange={checked => setNavAutoSubmit(!!checked)} />
        <div>
          <p className="text-sm font-medium text-foreground">Automatikus beküldés</p>
          <p className="text-xs text-muted-foreground">Lezárt bevallások automatikus beküldése a NAV felé (KR csatornán)</p>
        </div>
      </label>
