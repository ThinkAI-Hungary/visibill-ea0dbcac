    ...
    <div className="w-full max-w-5xl mx-auto space-y-6 page-animate">
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <h1 className="text-2xl font-bold text-foreground tracking-tight">Iratkezelés és GDPR</h1>
          <p className="text-xs text-muted-foreground mt-1 font-normal">Megőrzési szabályzat, adatfeldolgozói szerződések, érintetti kérelmek</p>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {rulesLoading ? (
            ...
            <div className="bg-card rounded-lg border border-border p-12 text-center space-y-3">
              ...
              <p className="text-sm text-muted-foreground">Nincsenek megőrzési szabályok beállítva ehhez a céghez.</p>
            ...
            <div className="bg-card rounded-lg border border-border shadow-soft overflow-hidden">
              <div className="px-5 py-3 border-b border-border dark:bg-card/30 flex items-center justify-between">
                <h2 className="text-sm font-bold text-foreground/90">Dokumentumtípusok és megőrzési idők</h2>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border dark:bg-card/20">
                      <th className="text-left px-5 py-2.5 text-xs font-bold text-muted-foreground uppercase">Dokumentumtípus</th>
                      <th className="text-center px-3 py-2.5 text-xs font-bold text-muted-foreground uppercase">Megőrzés</th>
                      <th className="text-left px-3 py-2.5 text-xs font-bold text-muted-foreground uppercase">Jogalap</th>
                      <th className="text-center px-3 py-2.5 text-xs font-bold text-muted-foreground uppercase">Auto törlés</th>
                      <th className="text-center px-3 py-2.5 text-xs font-bold text-muted-foreground uppercase">Műveletek</th>
                    </tr>
                  </thead>
                  <tbody>
                    {(rules || []).map((rule) => (
                      <tr key={rule.id} className="border-b border-border/50 hover:bg-muted/50 transition-colors">
                        <td className="px-5 py-3 font-medium text-foreground">{rule.docType}</td>
                        ...
                        <td className="px-3 py-3 text-center">
                          {editingRuleId === rule.id ? (
                            <Checkbox
                              ...
                              onCheckedChange={checked => setEditValues(v => ({ ...v, autoDelete: !!checked }))}
                            />
                          ) : (
                            <div className={cn(
                              'w-5 h-5 rounded-full mx-auto flex items-center justify-center',
                              rule.autoDelete ? 'bg-emerald-100 dark:bg-emerald-500/20' : 'bg-muted'
                            )}>
                              {rule.autoDelete ? <CheckCircle className="w-3.5 h-3.5 text-emerald-600" /> : <span className="text-muted-foreground text-[10px]">—</span>}
                            </div>
                          )}
                        </td>
                        <td className="px-3 py-3 text-center">
                      ...
                      <tr className="border-b border-border/50 bg-blue-50/50 dark:bg-blue-500/5">
                        ...
                        <td className="px-3 py-3 text-center">
                          <Checkbox
                            ...
                            onCheckedChange={checked => setNewRule(r => ({ ...r, autoDelete: !!checked }))}
                          />
                        </td>
                        <td className="px-3 py-3 text-center">
                          <div className="flex items-center justify-center gap-1">
            ...
            <>
              ...
              {showAddContract ? (
                <div className="bg-card rounded-lg border border-border p-5 space-y-4">
                    ...
                    {contractFile ? (
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <div className="text-left">
                          <p className="text-sm font-medium text-foreground">{contractFile.name}</p>
                      ...
                      <>
                        ...
                        <p className="text-sm font-medium text-muted-foreground dark:text-foreground/90">Szerződés feltöltése</p>
                        <p className="text-xs text-muted-foreground mt-1">Drag & drop vagy kattintson ide (PDF, DOC)</p>
                      </>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs text-muted-foreground mb-1 block">Partner neve *</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="text-xs text-muted-foreground mb-1 block">Érvényes eddig</label>
                      <DatePicker
                        value={newContract.validUntil}
                        onChange={val => setNewContract(c => ({ ...c, validUntil: val }))}
              ...
              <div className="bg-card rounded-lg border border-border shadow-soft overflow-hidden">
                <div className="px-5 py-3 border-b border-border dark:bg-card/30">
                  <h2 className="text-sm font-bold text-foreground/90">Adatfeldolgozói szerződések (GDPR 28. cikk)</h2>
                </div>
                <div className="divide-y divide-border/50">
                  {(contracts || []).length === 0 ? (
                    <div className="py-12 text-center text-sm text-muted-foreground">Nincs rögzített szerződés</div>
                  ) : (contracts || []).map(c => (
                    <div key={c.id} className="flex items-center gap-4 px-5 py-4 hover:bg-muted/50 transition-colors">
                      <div className={cn(
