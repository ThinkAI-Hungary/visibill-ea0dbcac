  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6 page-animate">
      <div className="flex items-center gap-3">
        ...
        <div className="p-2.5 bg-gradient-to-br from-indigo-500 to-violet-600 rounded-lg shadow-lg shadow-primary/20"><Send className="w-5 h-5 text-white" /></div>
        <div>
          <h1 className="text-2xl font-bold">Bevallás beküldés</h1>
          <p className="text-sm text-muted-foreground">{filingData?.typeLabel || 'Betöltés...'} — {filingData?.year}/{filingData?.month ? String(filingData.month).padStart(2, '0') : '–'}</p>
        </div>
      </div>
        ...
        {STEPS.map((s, i) => (
          <React.Fragment key={s.id}>
            <button onClick={() => i <= stepIndex && setStep(s.id)} className={cn(
              ...
              step === s.id ? 'bg-primary text-white ring-2 ring-primary/40' :
              'bg-muted text-muted-foreground dark:bg-muted'
            )}>
              {stepIndex > i ? <CheckCircle className="w-3.5 h-3.5" /> : <s.icon className="w-3.5 h-3.5" />}
              {s.label}
            </button>
            {i < STEPS.length - 1 && <div className={cn('w-8 h-0.5', stepIndex > i ? 'bg-emerald-500' : 'bg-muted')} />}
          </React.Fragment>
        ))}
      </div>
      ...
      {step === 'preview' && (
        <div className="space-y-4">
          <div className="bg-card rounded-lg border border-border p-6">
            ...
            <div className="bg-card border border-border rounded-lg p-8 min-h-[300px] font-mono text-xs space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-xs">
                <div><span className="text-muted-foreground">Adóalany neve:</span> <strong>{company?.name || '–'}</strong></div>
                <div><span className="text-muted-foreground">Adószám:</span> <strong>{company?.taxNumber || '–'}</strong></div>
              </div>
              {filingData?.xmlMeta && (
                  ...
                  <div className="grid grid-cols-3 gap-3 text-xs">
                    {filingData.xmlMeta.bevallasIdoszak && (
                      <div className="bg-muted/50 rounded-lg p-3">
                        <p className="text-[10px] text-muted-foreground uppercase font-bold">Bevallási időszak</p>
                        <p className="font-bold mt-0.5">{filingData.xmlMeta.bevallasIdoszak}</p>
                      </div>
                    )}
                    {filingData.xmlMeta.bevallasTipus && (
                      <div className="bg-muted/50 rounded-lg p-3">
                        <p className="text-[10px] text-muted-foreground uppercase font-bold">Típus</p>
                    ...
                    {filingData.xmlMeta.benyujtasDatuma && (
                      <div className="bg-muted/50 rounded-lg p-3">
                        <p className="text-[10px] text-muted-foreground uppercase font-bold">Benyújtás dátuma</p>
                        ...
                        <table className="w-full text-[11px]">
                          <thead>
                            <tr className="bg-muted/50 border-b border-border">
                              <th className="px-3 py-2 text-left font-bold text-muted-foreground">Név</th>
                              <th className="px-3 py-2 text-left font-bold text-muted-foreground">TAJ szám</th>
                              <th className="px-3 py-2 text-right font-bold text-muted-foreground">Bruttó</th>
                              <th className="px-3 py-2 text-right font-bold text-muted-foreground">SZJA</th>
                              <th className="px-3 py-2 text-right font-bold text-muted-foreground">TB</th>
                              <th className="px-3 py-2 text-right font-bold text-muted-foreground">SZOCHO</th>
                              <th className="px-3 py-2 text-right font-bold text-muted-foreground">Nettó</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-border/50">
                            {filingData.xmlEmployees.map((emp, i) => (
                              <tr key={i} className="hover:bg-muted/50 dark:hover:bg-muted/50/30">
                                <td className="px-3 py-2 font-medium">{emp.name}</td>
                                <td className="px-3 py-2 font-mono text-muted-foreground">{emp.taj}</td>
          ...
          <div className="flex justify-between">
            ...
            <Button onClick={() => setStep('sign')} className="gap-1.5 bg-primary hover:bg-primary/90">
      ...
      {step === 'sign' && (
        <div className="space-y-4">
          <div className="bg-card rounded-lg border border-border p-8 text-center space-y-6">
            <Stamp className="w-16 h-16 mx-auto text-primary" />
            <div>
              <h2 className="text-lg font-bold">Elektronikus aláírás (AVDH)</h2>
              <p className="text-sm text-muted-foreground mt-1">A bevallás beküldéséhez AVDH (Azonosításra Visszavezetett Dokumentumhitelesítés) szükséges.</p>
            </div>
            ...
            <Button onClick={handleSign} disabled={signing} className="gap-1.5 bg-primary hover:bg-primary/90">
        ...
        <div className="space-y-4">
          <div className="bg-card rounded-lg border border-border p-8 text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">Az elektronikus aláírás sikeres. A bevallás beküldése a NAV ÁNYK rendszerébe történik.</p>
            </div>
            <div className="bg-card rounded-lg border border-border p-4 text-left text-sm grid grid-cols-2 gap-3">
              <div><span className="text-muted-foreground">Bevallás típus:</span> <strong>{filingData?.typeLabel || '–'}</strong></div>
              <div><span className="text-muted-foreground">Időszak:</span> <strong>{filingData?.year}/{filingData?.month ? String(filingData.month).padStart(2, '0') : '–'}</strong></div>
              <div><span className="text-muted-foreground">Cég:</span> <strong>{company?.name || '–'}</strong></div>
              <div><span className="text-muted-foreground">Aláírás időpont:</span> <strong>{new Date().toLocaleString('hu-HU')}</strong></div>
            </div>
      ...
      {step === 'receipt' && (
        <div className="space-y-4">
          <div className="bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-500/10 dark:to-green-500/10 rounded-lg border border-emerald-200 dark:border-emerald-500/20 p-10 text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-2">A NAV rendszere befogadta a bevallást.</p>
            </div>
            <div className="bg-card rounded-lg border border-border p-5 text-left text-sm max-w-md mx-auto space-y-2">
              <div className="flex justify-between"><span className="text-muted-foreground">Hivatkozási szám:</span><span className="font-mono font-bold">{receiptId || filing?.nav_receipt_id || '–'}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Beküldés időpont:</span><span>{filing?.submitted_at ? new Date(filing.submitted_at).toLocaleString('hu-HU') : new Date().toLocaleString('hu-HU')}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Státusz:</span><span className="text-emerald-600 font-bold">Befogadva</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Feldolgozás várható:</span><span>24 órán belül</span></div>
            </div>
