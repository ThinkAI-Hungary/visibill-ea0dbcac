    ...
    return (
      <div className="w-full max-w-3xl mx-auto space-y-6 page-animate">
        <div className="flex items-center gap-3">
          ...
          <div className="p-2.5 bg-gradient-to-br from-red-500 to-pink-600 rounded-lg shadow-lg"><UserX className="w-5 h-5 text-white" /></div>
          <h1 className="text-2xl font-bold">Kilépési varázsló</h1>
        </div>
        <div className="bg-card rounded-lg border border-border p-12 text-center text-sm text-muted-foreground">Nincs aktív jogviszony ehhez a munkavállalóhoz.</div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-6 page-animate">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">Kilépési varázsló</h1>
          <p className="text-sm text-muted-foreground">{activeJob.job_title || activeJob.employment_type} — {activeJob.feor_code}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        {STEPS.map((s, i) => (
          <React.Fragment key={i}>
            ...
            {i < STEPS.length - 1 && <ArrowRight className="w-3 h-3 text-muted-foreground/60" />}
          </React.Fragment>
        ))}
      </div>
      ...
      {step === 0 && (
        <div className="bg-card rounded-lg border border-border p-6 space-y-4">
          <h2 className="text-sm font-bold text-foreground/90">Jogviszony megszűnésének oka</h2>
          <div className="grid gap-2">
            {REASONS.map(r => (
              <button key={r} onClick={() => setReason(r)} className={cn('p-3 rounded-lg border-2 text-left text-sm font-medium transition-all', reason === r ? 'border-red-500 bg-red-50 dark:bg-red-500/10' : 'border-border hover:border-red-300')}>
                {r}
              </button>
            ))}
          </div>
      ...
      {step === 1 && (
        <div className="bg-card rounded-lg border border-border p-6 space-y-4">
          <h2 className="text-sm font-bold text-foreground/90 flex items-center gap-2"><Calendar className="w-4 h-4" /> Utolsó munkanap</h2>
          <DatePicker
            value={lastDay}
            onChange={setLastDay}
      ...
      {step === 2 && (
        <div className="bg-card rounded-lg border border-border p-6 space-y-4">
          <h2 className="text-sm font-bold text-foreground/90">Pénzügyi elszámolás</h2>
          <div>
          ...
          <div className="flex items-center justify-between p-4 rounded-lg border border-border">
            ...
            <button onClick={() => setSeverancePay(!severancePay)} className={cn('relative w-12 h-6 rounded-full transition-colors', severancePay ? 'bg-emerald-500' : 'bg-muted-foreground/30')}>
          ...
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">Megjegyzés</label>
      ...
      {step === 3 && (
        <div className="bg-card rounded-lg border border-border p-6 space-y-4">
          <h2 className="text-sm font-bold text-foreground/90">Kilépő dokumentumcsomag</h2>
          <p className="text-xs text-muted-foreground">Jelöld be, mely dokumentumok készültek el vagy relevánsak.</p>
          <div className="space-y-2">
            {EXIT_DOCS.map(doc => (
              <button key={doc.id} onClick={() => setCheckedDocs(prev => {
                ...
                return next;
              })} className={cn('w-full flex items-center gap-3 p-3 rounded-lg border-2 text-left text-sm transition-all',
                checkedDocs.has(doc.id) ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10' : 'border-border hover:border-border'
              )}>
                <div className={cn('w-5 h-5 rounded flex items-center justify-center shrink-0',
                  checkedDocs.has(doc.id) ? 'bg-emerald-500 text-white' : 'border border-border'
                )}>
                ...
                <span className="text-[10px] text-muted-foreground font-mono">{doc.ref}</span>
              </button>
            ))}
          </div>
          <p className="text-[10px] text-muted-foreground">{checkedDocs.size}/{EXIT_DOCS.length} dokumentum jelölve</p>
        </div>
      )}
      ...
      {step === 4 && (
        <div className="bg-card rounded-lg border border-border p-6 space-y-4">
          <h2 className="text-sm font-bold text-foreground/90">Összesítés</h2>
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div><p className="text-muted-foreground text-xs">Munkavállaló:</p><p className="font-bold">{activeJob.job_title || activeJob.employment_type}</p></div>
            <div><p className="text-muted-foreground text-xs">Kilépés oka:</p><p className="font-bold">{reason}</p></div>
            <div><p className="text-muted-foreground text-xs">Utolsó munkanap:</p><p className="font-bold">{lastDay}</p></div>
            <div><p className="text-muted-foreground text-xs">Szabadság megváltás:</p><p className="font-bold">{leavePayDays} nap ({leavePayAmount.toLocaleString('hu-HU')} Ft)</p></div>
            <div><p className="text-muted-foreground text-xs">Végkielégítés:</p><p className="font-bold">{severancePay ? 'Igen' : 'Nem'}</p></div>
            <div><p className="text-muted-foreground text-xs">Dokumentumok:</p><p className="font-bold">{checkedDocs.size}/{EXIT_DOCS.length} kész</p></div>
          </div>
          {notes && <div className="bg-muted/50 rounded-lg p-3 text-xs text-muted-foreground">{notes}</div>}
